...
function StepBadge({ label, theme }: { label: string; theme?: string | undefined }) {
  return (
    <span
      style={{
...
function SectionTitle({ text, theme }: { text: string; theme?: string | undefined }) {
  return (
    <h2
      style={{
        fontSize: 17,
...
function validateContactFields(contact: {
  ...
  if (!contact.phone && !contact.email)
    return 'Please enter at least a phone number or email so we can confirm your booking.'
  if (contact.phone && !/^[\d\s+\-().]{7,}$/.test(contact.phone))
    return 'Please enter a valid phone number (digits only, e.g. +1 234 567 8900).'
...
function validateBookingTime(
  ...
  if (isTooSoonMinutes(slotMin, nowMin, minAdvance, leadEnabled))
    return `Reservá con al menos ${minAdvance} minutos de anticipación.`
  return null
}
  ...
  if (res.status === 400) {
    ...
    if (err === 'too_soon') {
      ...
      actions.setBookingError(
        msg ?? `Reservá con al menos ${actions.minAdvance} minutos de anticipación.`,
      )
      return { handled: true }
    }
  }
...
function LocationSelector({
  ...
  function handleChange(val: string | null) {
    ...
    if (selectedService?.location_id && val && selectedService.location_id !== val)
...
function ServiceList({
  ...
  if (services.length === 0)
    return <p style={{ fontSize: 14, color: cardMuted }}>{t('selectService.empty')}</p>
  return (
    <>
      {services.map((s) => (
        <button type="button" key={s.id} onClick={() => onSelect(s)} style={baseCard}>
          <div>
            ...
            {s.description && (
              <div style={{ fontSize: 12, color: cardMuted, marginTop: 2 }}>{s.description}</div>
            )}
...
function SlotGrid({
  ...
  slotSpotsLeft: Record<string, number>
  capacity: number
  formatSlot: (s: string) => string
  theme?: string | undefined
  cardText: string
}) {
  const isEsc = theme === 'escuderia'
...
function TimeSlotsSection({
  ...
  if (loadingSlots)
    return (
      <div
        style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: cardMuted }}
      >
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, color: cardMuted, marginBottom: 8 }}>
        Unite a la lista de espera. Te avisamos por WhatsApp cuando se libere un slot (30m para
        confirmar).
      </div>
      <button
        type="button"
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: cardMuted, marginTop: -8, marginBottom: 16 }}>
        {service.name} · {service.duration_min} min{employeeName && ` · ${employeeName}`}
      </p>
      <div style={{ marginBottom: 16 }}>
        <label
          style={{
      ...
      {showWaitlist && (
        <WaitlistPrompt
          ...
          cardText={cardText}
          cardMuted={cardMuted}
          onJoin={onJoinWaitlistFromDatetime}
        />
      )}
      <CtaButton
...
function ContactInputs({
  ...
  setContact: (
    c: (prev: { name: string; phone: string; email: string }) => {
      name: string
      phone: string
      email: string
    },
    ...
    <div style={{ borderTop: '0.5px solid #E8E0D8', paddingTop: 14, marginTop: 4 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div>
          ...
          />
          {loyaltyBalance !== null && (
            <span style={{ fontSize: 11, color: cardMuted }}>Tienes {loyaltyBalance} pts</span>
          )}
        </div>
      </div>
    </div>
...
function SubmitBookingButton({
  ...
  return (
    ...
    >
      {saving
        ? t('contact.booking')
        : t('contact.confirm', { price: formatCurrency(price, businessCurrency) })}
    </button>
  )
}
  ...
  return (
    <div>
      ...
      <WaitlistActionButton
        contactName={contact.name}
        waitlistJoinLoading={waitlistJoinLoading}
...
function DoneScreen({
  ...
  const telegramLink =
    telegramBotUsername && clientId
      ...
      : null
  const viberLink =
    viberBotUri && clientId ? `viber://pa?chatURI=${viberBotUri}&context=client_${clientId}` : null
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 14, color: textMuted, margin: '0 0 4px' }}>
        {service?.name} · {date} at {time ? formatSlot(time) : ''}
...
function hasEmployeeStepForLocation(
  employees: Employee[],
  selectedLocation: string | null,
): boolean {
...
function BookingSteps({
  ...
  handleBackFromEmployee: () => void
  handleBackFromDatetime: () => void
  handleDatetimeContinue: () => void
  date: string
  time: string
  availableSlots: string[]
  slotSpotsLeft: Record<string, number>
  ...
  if (step === 'employee' && selectedService)
    return (
  ...
  if (step === 'datetime' && selectedService)
    return (
      <DateTimeSelectionStep
        ...
        cardMuted={cardMuted}
        isEsc={isEsc}
        onJoinWaitlistFromDatetime={() => setStep('contact')}
      />
    )
  if (step === 'contact')
    return (
      <div>
        ...
        <ContactFormStep
          contact={contact}
          setContact={setContact as never}
          promoCode={promoCode}
          setPromoCode={setPromoCode}
          loyaltyPoints={loyaltyPoints}
          setLoyaltyPoints={setLoyaltyPoints}
          ...
          cardMuted={cardMuted}
          isEsc={isEsc}
          inputBg={isEsc ? '#1c1b1b' : 'white'}
          inputBorder={isEsc ? '1px solid rgba(197,160,89,0.2)' : '0.5px solid #E8E0D8'}
          inputRadius={isEsc ? 0 : 10}
        />
      </div>
    )
  return null
...
export function PublicBookingForm({
  ...
  const [, setHolidaysForLocation] = useState<
  ...
  const todayStr = useMemo(
    () => todayInBusinessTzLib(business.timezone ?? 'UTC', new Date()),
    [business.timezone],
  )
  const [today, setToday] = useState(todayStr)
  useEffect(
    () => setToday(todayInBusinessTzLib(business.timezone ?? 'UTC', new Date())),
    [business.timezone],
  )
  const nowMin = useMemo(
    () => nowMinutesInBusinessTzLib(business.timezone ?? 'UTC', new Date()),
    [business.timezone],
  )
      ...
      try {
        ...
        if (user) {
          ...
          if (linkedClient)
            setContact((prev) => ({
              name:
                (linkedClient as { name: string }).name ??
                prev.name ??
                ((user.user_metadata as Record<string, unknown>)?.name as string) ??
                user.email?.split('@')[0] ??
                '',
              phone:
                (linkedClient as { phone: string | null }).phone ?? prev.phone ?? user.phone ?? '',
              email:
                (linkedClient as { email: string | null }).email ?? prev.email ?? user.email ?? '',
            }))
          else
            setContact((prev) => ({
              name:
  ...
  const loadSlots = useCallback(
    async (selectedDate: string, svc: Service, employeeId: string): Promise<void> => {
      ...
      try {
        ...
        const { filtered, spotsMap } = computeAvailableSlotsWithCapacity(
          slots,
          booked as { starts_at: string; ends_at: string }[] | null,
          capacity,
          svc,
  ...
  const submit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const handled = await handleBookingResponse(res, {
        setBookingError,
        setSlotTakenError,
        setTime,
        setStep,
        loadSlots: reload,
        t: (k: string) => String(t(k as never)),
        minAdvance,
      })
      if (handled.handled) {
        setSaving(false)
  ...
  const handleSelectService = useCallback(
    (s: Service) => {
      ...
      setStep(hasEmployeeStepForLocation(employees, selectedLocation) ? 'employee' : 'datetime')
    },
  ...
  const handleBackFromDatetime = useCallback(
    () => setStep(hasEmployeeStepForLocation(employees, selectedLocation) ? 'employee' : 'service'),
    [employees, selectedLocation],
  )
  ...
  if (step === 'done')
    return (
      <DoneScreen
        ...
        clientHasTelegram={clientHasTelegram}
        onReset={resetAll}
        theme={theme}
        formatSlot={formatSlot}
  ...
  return (
    <BookingSteps
      ...
      handleBackFromEmployee={handleBackFromEmployee}
      handleBackFromDatetime={handleBackFromDatetime}
      handleDatetimeContinue={handleDatetimeContinue}
      date={date}
      time={time}
      availableSlots={availableSlots}
      slotSpotsLeft={slotSpotsLeft}
      ...
      formatSlot={formatSlot}
      theme={theme}
      t={t as (k: string, o?: unknown) => string}
      cardText={cardText}
      cardMuted={cardMuted}
      isEsc={isEsc}
      contact={contact}
      ...
      setDate={setDate}
      setTime={setTime}
      setSlotTakenError={setSlotTakenError}
      setStep={setStep}
    />
  )
}
