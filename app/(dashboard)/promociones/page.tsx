...
export default async function PromocionesPage(props: { searchParams: Promise<{ location?: string }> }) {
  ...
  let promoQuery = supabase.from('promotions').select('id, name, type, value, promo_code, valid_from, valid_to, rules, is_active, location_id, created_at').eq('business_id', businessId).order('created_at', { ascending: false })
  if (selectedLocation) promoQuery = (promoQuery as unknown as { eq: (c:string,v:string)=> typeof promoQuery }).eq('location_id', selectedLocation) as typeof promoQuery
  const [{ data: promotions }, { data: locations }, { data: services }] = await Promise.all([
    promoQuery as Promise<{ data: unknown }>,
    ...
    <>
      ...
      {(locations?.length ?? 0) > 1 && (
        <div className="px-6 pt-3 flex gap-2 text-xs">
          <Link href="/promociones" className={`px-3 py-1 rounded-full border ${!selectedLocation ? 'bg-gray-900 text-white' : 'bg-white'}`}>Todas</Link>
          {locations!.map((l) => (
            <Link key={l.id} href={`/promociones?location=${l.id}`} className={`px-3 py-1 rounded-full border ${selectedLocation === l.id ? 'bg-gray-900 text-white' : 'bg-white'}`}>{l.name}</Link>
          ))}
        </div>
      )}
