  ...
  const [{ data: services }, { data: employees }, { data: businessHours }, { data: locations }] =
    await Promise.all([
      ...
      supabase
        .from('employees')
        .select(
          'id, name, role, email, phone, is_active, color, specialties, commission_rate, commission_fixed, bio, avatar_url',
        )
        .eq('business_id', business.id)
        .order('name'),
      supabase
        ...
        <ConfigSection
          ...
          initial={{
            ...
            payment_methods: (business as unknown as { payment_methods?: string[] })
              .payment_methods,
            cancel_lead_time: (business as unknown as { cancel_lead_time?: number })
              .cancel_lead_time,
            min_advance_minutes: business.min_advance_minutes,
            booking_lead_time_enabled: business.booking_lead_time_enabled ?? true,
            loyalty_earn_rate: (business as unknown as { loyalty_earn_rate?: number })
              .loyalty_earn_rate,
            loyalty_redeem_rate: (business as unknown as { loyalty_redeem_rate?: number })
              .loyalty_redeem_rate,
            loyalty_redeem_value: (business as unknown as { loyalty_redeem_value?: number })
