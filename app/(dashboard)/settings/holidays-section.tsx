    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid gap-3 mb-4 p-4 border border-gray-100 rounded-xl bg-gray-50">
          ...
          {!hasMultipleLocations && (
            <div className="flex items-end gap-2">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" checked={form.is_open} onChange={(e) => setForm((f) => ({ ...f, is_open: e.target.checked }))} className="accent-blue-600" />
        ...
        {hasMultipleLocations && (
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={form.is_open} onChange={(e) => setForm((f) => ({ ...f, is_open: e.target.checked }))} className="accent-blue-600" />
            <span className="text-xs text-gray-600">Marcar como abierto aunque sea festivo (excepción)</span>
          </label>
        )}
        <div>
          ...
          <span className="text-xs text-gray-400 ml-3">is_open=false bloquea el picker y /api/book</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-6 text-center text-sm text-gray-500 border border-dashed border-gray-200 rounded-xl">Sin bloqueos. Agregá festivos o cierres por mantenimiento.</div>
      ) : (
            ...
            <tbody>
              {holidays.map((h) => (
                <tr key={h.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  {hasMultipleLocations && <td className="px-4 py-3 text-gray-600">{h.location_id ? locations.find((l) => l.id === h.location_id)?.name ?? h.location_id.slice(0, 8) : 'Todas'}</td>}
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${h.is_open ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
                      {h.is_open ? 'Abierto' : 'Cerrado'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => remove(h.id)} className="p-1.5 hover:bg-red-50 rounded text-red-400 hover:text-red-600">
      ...
      <p className="text-xs text-gray-400 mt-3">Los días marcados como cerrado no aparecen como slots en <code>/book/[slug]</code> y son rechazados por <code>/api/book</code> con <code>outside_availability:holiday</code>.</p>
    </div>
  )
}
