...
export default async function ServiciosPage(props: { searchParams: Promise<{ location?: string }> }) {
  ...
  let svcQuery = supabase.from('services').select('id, name, description, price, duration_min, category, is_active, is_featured, color, cost, location_id').eq('business_id', businessId).order('name')
  if (selectedLocation) svcQuery = (svcQuery as unknown as { eq: (c:string,v:string)=> typeof svcQuery }).eq('location_id', selectedLocation) as typeof svcQuery
  ...
  const [{ data: services }, { data: locations }] = await Promise.all([
    svcQuery as Promise<{ data: unknown }>,
    ...
    <>
      ...
      {(locations?.length ?? 0) > 1 && (
        <div className="px-6 pt-3 flex gap-2 text-xs">
          <Link href="/servicios" className={`px-3 py-1 rounded-full border ${!selectedLocation ? 'bg-gray-900 text-white' : 'bg-white'}`}>Todos</Link>
          {locations!.map((l) => (
            <Link key={l.id} href={`/servicios?location=${l.id}`} className={`px-3 py-1 rounded-full border ${selectedLocation === l.id ? 'bg-gray-900 text-white' : 'bg-white'}`}>{l.name}</Link>
          ))}
        </div>
      )}
      <main className="p-6">
