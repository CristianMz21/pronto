...
export default async function DashboardPage(props: { searchParams: Promise<{ location?: string }> }) {
  ...
  const [
    ...
    (() => { let q = supabase.from('transactions').select('id, amount, payment_method, created_at, clients(name), employee_id, location_id').eq('business_id', business.id).eq('status', 'completed').order('created_at', { ascending: false }).limit(5); if (selectedLocation) q = (q as unknown as { eq: (c: string, v: string) => typeof q }).eq('location_id', selectedLocation) as typeof q; return q })() as Promise<{ data: unknown }>,
    ...
    (() => { let q = supabase.from('transactions').select('client_id, location_id').eq('business_id', business.id).eq('status', 'completed').limit(1000); if (selectedLocation) q = (q as unknown as { eq: (c: string, v: string) => typeof q }).eq('location_id', selectedLocation) as typeof q; return q })(),
  ])
