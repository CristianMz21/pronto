...
export default async function CajaPage(props: { searchParams: Promise<{ location?: string }> }) {
  ...
  if (owned) business = owned as { id: string; currency: string }
    ...
    if (empBiz) { const b = (empBiz as unknown as { businesses: { id: string; currency: string } }).businesses; business = b }
  }
  ...
  let openQuery = supabase
    ...
    .eq('status', 'open')
  if (searchParams.location) openQuery = openQuery.eq('location_id', searchParams.location) as typeof openQuery
  ...
  let historyQuery = supabase
    .from('cash_registers')
    .select('id, opening_cash, expected_cash, actual_cash, difference, status, opened_at, closed_at, notes, location_id')
    .eq('business_id', business.id)
    .order('opened_at', { ascending: false })
    .limit(10)
  if (searchParams.location) historyQuery = historyQuery.eq('location_id', searchParams.location) as typeof historyQuery
