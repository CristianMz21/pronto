...
export async function POST(req: NextRequest) {
  const ip = getIp(req)
  if (!rateLimit(`wa-verify:${ip}`, { limit: 10, windowMs: 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  ...
  if (body.verify) {
    if (!phoneId || !token) return NextResponse.json({ error: 'missing_credentials', message: 'phone_number_id and access_token required' }, { status: 400 })
