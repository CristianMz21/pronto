...
function sanitize(s: string): string { return DOMPurify.sanitize(s, { ALLOWED_TAGS: [] }).trim() }
...
export async function GET(req: NextRequest) {
  ...
  const { data: biz } = await supabase
    .from('businesses')
    .select('tax_rate, payment_methods, cancel_lead_time, min_advance_minutes, booking_lead_time_enabled, loyalty_earn_rate, loyalty_redeem_rate, loyalty_redeem_value')
    .eq('id', businessId)
    .maybeSingle()
  ...
  if (locationId) {
    const { data: loc } = await supabase.from('locations').select('id, name').eq('id', locationId).eq('business_id', businessId).maybeSingle()
    if (!loc) return NextResponse.json({ error: 'location_not_found' }, { status: 404 })
  }

  // Also fetch business_settings for completeness
  const { data: settings } = await supabase.from('business_settings').select('tax_rate, payment_methods, cancel_lead_time').eq('business_id', businessId).maybeSingle()

  return NextResponse.json({ business: biz, business_settings: settings, location_id: locationId || null })
}

export async function PUT(req: NextRequest) {
  const ip = getIp(req)
  if (!rateLimit(`business-tax:${ip}`, { limit: 30, windowMs: 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthorized' }, { status: 401 })
  const businessId = await resolveBusinessId(supabase, user.id)
  ...
  if (b.payment_methods !== undefined) patch.payment_methods = b.payment_methods.map((m) => sanitize(m))
  ...
  if (b.business_lead_time_enabled !== undefined) patch.booking_lead_time_enabled = b.business_lead_time_enabled
  ...
  if (Object.keys(settingsPatch).length > 0) {
    await supabase.from('business_settings').upsert({ business_id: businessId, ...settingsPatch } as unknown as never, { onConflict: 'business_id' } as unknown as never)
  }
