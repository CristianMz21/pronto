...
const DayHoursSchema = z.object({
  ...
  break_start: z.string().regex(/^\d{2}:\d{2}$/).nullable().optional().or(z.literal('')),
  break_end: z.string().regex(/^\d{2}:\d{2}$/).nullable().optional().or(z.literal('')),
})
...
export async function PUT(req: NextRequest) {
  const ip = getIp(req)
  if (!rateLimit(`business-hours:${ip}`, { limit: 30, windowMs: 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthorized' }, { status: 401 })
  const businessId = await resolveBusinessId(supabase, user.id)
  ...
  for (const h of body.hours) {
    if (h.break_start && h.break_end) {
      if (h.break_start >= h.break_end) return NextResponse.json({ error: 'validation_failed', details: { break: [`break_start must be < break_end for day ${h.day_of_week}`] } }, { status: 422 })
      if (h.break_start < h.open_time || h.break_end > h.close_time) return NextResponse.json({ error: 'validation_failed', details: { break: [`break outside open/close for day ${h.day_of_week}`] } }, { status: 422 })
    }
  }
  ...
  const { error } = await supabase.from('business_hours').upsert(rows, { onConflict: 'business_id,location_id,day_of_week' } as unknown as { onConflict: string })
  // Fallback if constraint not yet satisfied (location_id null handling)
  if (error) {
    // Try delete + insert for that location
    if (locationId) await supabase.from('business_hours').delete().eq('business_id', businessId).eq('location_id', locationId)
    else await supabase.from('business_hours').delete().eq('business_id', businessId).is('location_id', null)
