...
export async function POST(req: NextRequest) {
  ...
  try {
    const campaign = await createFromSegment(supabase as unknown as Parameters<typeof createFromSegment>[0], {
      businessId,
    ...
    const { count } = await supabase.from('campaign_recipients').select('client_id', { count: 'exact', head: true }).eq('campaign_id', campaign.id)
