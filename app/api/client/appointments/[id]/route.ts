...
import { isPastInTz, isTooSoonInTz, parseDateTimeInTz, DEFAULT_LEAD_MINUTES } from '@/lib/booking-availability'
...
export async function PATCH(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  if (isPastInTz(starts, new Date())) {
    return NextResponse.json({ error: 'in_past', message: 'No se puede cancelar una cita pasada' }, { status: 400 })
  }
...
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(time)) {
      return NextResponse.json({ error: 'validation_failed', message: 'Invalid date or time' }, { status: 422 })
    }
  } catch {
  ...
  if (isPastInTz(new Date(appt.starts_at), new Date())) {
    return NextResponse.json({ error: 'in_past', message: 'No se puede reprogramar una cita pasada' }, { status: 400 })
  }
  ...
  if (isPastInTz(newStarts, now)) {
    return NextResponse.json({ error: 'in_past', message: 'No se puede reprogramar en el pasado' }, { status: 400 })
  }
  ...
  const { error: updErr } = await supabase.from('appointments')
    .update({ starts_at: newStarts.toISOString(), ends_at: newEnds.toISOString(), status: 'confirmed' })
    .eq('id', id)

  if (updErr) {
    if (updErr.message.includes('slot_already_booked')) {
      return NextResponse.json({ error: 'slot_taken', message: 'Ese horario ya está ocupado' }, { status: 409 })
    }
