...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from('services').select('id, name, description, price, duration_min, category, is_active, is_featured, color, cost, location_id, created_at').eq('business_id', businessId).order('name')
...
export async function POST(request: Request) {
  ...
  if (!rateLimit(`services-create:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthorized' }, { status: 401 })
  const businessId = await resolveBusinessId(supabase, user.id)
