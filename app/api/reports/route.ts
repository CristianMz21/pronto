...
export async function GET(request: Request) {
  ...
  let q = supabase.from('transactions').select('amount, employee_id').eq('business_id', businessId).eq('status', 'completed').gte('created_at', since).limit(1000)
  if (location) q = (q as unknown as { eq: (c: string, v: string) => typeof q }).eq('location_id', location) as typeof q
  const { data, error } = await q
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  const byBarber = reportSalesByBarber((data ?? []) as { amount: number; employee_id: string | null }[])
