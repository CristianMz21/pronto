...
export async function PATCH(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const ip = getIp(req)
  if (!rateLimit(`promotions-patch:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  ...
  const b = parsed.data
  if (b.type === 'percent' && b.value != null && Number(b.value) > 100) return NextResponse.json({ error: 'validation_failed', details: { value: ['percent max 100'] } }, { status: 422 })
  ...
  if (b.promo_code !== undefined) payload.promo_code = b.promo_code ? (b.promo_code as string).toUpperCase() : null
  ...
  const { data, error } = await supabase.from('promotions').update(payload).eq('id', id).eq('business_id', businessId).select('id').single()
...
export async function DELETE(_req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const id = params.id
  if (!z.string().uuid().safeParse(id).success) return NextResponse.json({ error: 'invalid_id' }, { status: 400 })
  const { error } = await supabase.from('promotions').update({ is_active: false }).eq('id', id).eq('business_id', businessId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
