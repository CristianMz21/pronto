...
export async function GET() {
  ...
  const { data, error } = await supabase.from('promotions').select('id, business_id, location_id, name, type, value, promo_code, valid_from, valid_to, rules, is_active, created_at').eq('business_id', businessId).order('created_at', { ascending: false })
...
export async function POST(req: Request) {
  const ip = getIp(req)
  if (!rateLimit(`promotions-create:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthorized' }, { status: 401 })
  const businessId = await resolveBusinessId(supabase, user.id)
  ...
  if (b.type === 'percent' && Number(b.value) > 100) return NextResponse.json({ error: 'validation_failed', details: { value: ['percent max 100'] } }, { status: 422 })
  if (b.valid_from && b.valid_to) {
    ...
    if (!isNaN(from.getTime()) && !isNaN(to.getTime()) && from.getTime() >= to.getTime()) {
      return NextResponse.json({ error: 'validation_failed', details: { valid_to: ['must be after valid_from'] } }, { status: 422 })
    }
  }
  ...
  if (error) {
    if (String(error.message).includes('duplicate') || String(error.message).includes('unique')) return NextResponse.json({ error: 'promo_code_duplicate' }, { status: 409 })
