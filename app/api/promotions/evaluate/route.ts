...
export async function POST(req: Request) {
  const ip = getIp(req)
  if (!rateLimit(`promotions-evaluate:${ip}`, { limit: 120, windowMs: 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  ...
  if (b.already_discounted) return NextResponse.json({ eligible: false, reason: 'promo_stack_guard', discount: 0 }, { status: 409 })
  ...
  if (b.promotion_id) {
    const { data } = await supabase.from('promotions').select('*').eq('id', b.promotion_id).eq('business_id', businessId).maybeSingle()
    promo = data as typeof promo
  } else if (b.promo_code) {
    const { data } = await supabase.from('promotions').select('*').eq('business_id', businessId).eq('promo_code', b.promo_code.toUpperCase()).maybeSingle()
    promo = data as typeof promo
  }
  ...
  if (!promo && !b.promotion_id && !b.promo_code) {
    ...
    const { data: all } = await supabase.from('promotions').select('*').eq('business_id', businessId).eq('is_active', true)
    let best: typeof promo = null
    let bestDiscount = 0
    let clientData: { birthday?: string | null; tags?: string[]; last_visit_at?: string | null; total_visits?: number } | null = null
    if (b.client_id) {
      const { data: c } = await supabase.from('clients').select('birthday, tags, last_visit_at, total_visits').eq('id', b.client_id).eq('business_id', businessId).maybeSingle()
      clientData = c as typeof clientData
    }
    for (const p of (all as typeof promo[]) ?? []) {
      ...
      const d = calculateDiscount(p as unknown as Parameters<typeof calculateDiscount>[0], b.amount ?? 0)
      if (d > bestDiscount) {
        bestDiscount = d
        best = p
      }
    ...
    return NextResponse.json({ eligible: true, promotion: best, discount: bestDiscount, finalAmount: Math.max(0, (b.amount ?? 0) - bestDiscount) })
  }

  if (!promo) return NextResponse.json({ eligible: false, reason: 'promo_not_found', discount: 0 }, { status: 404 })
  ...
  if (!evalRes.eligible) return NextResponse.json({ eligible: false, reason: evalRes.reason, discount: 0 })
  const discount = calculateDiscount(promo as unknown as Parameters<typeof calculateDiscount>[0], b.amount ?? 0)
  return NextResponse.json({ eligible: true, promotion: promo, discount, finalAmount: Math.max(0, (b.amount ?? 0) - discount) })
}
