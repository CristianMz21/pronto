...
export async function POST(req: Request) {
  const ip = getIp(req)
  if (!rateLimit(`memberships-purchase:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  ...
  const { data: client } = await supabase.from('clients').select('id').eq('id', client_id).eq('business_id', businessId).maybeSingle()
  if (!client) return NextResponse.json({ error: 'client_not_found' }, { status: 404 })

  try {
    const cm = await purchaseMembership(supabase as unknown as Parameters<typeof purchaseMembership>[0], {
