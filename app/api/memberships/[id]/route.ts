...
export async function PATCH(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const ip = getIp(req)
  if (!rateLimit(`memberships-patch:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  ...
  const { data, error } = await supabase.from('memberships').update(payload).eq('id', id).eq('business_id', businessId).select('id').single()
...
export async function DELETE(_req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabase.from('memberships').update({ is_active: false }).eq('id', id).eq('business_id', businessId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
