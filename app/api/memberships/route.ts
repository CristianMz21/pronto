...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('memberships')
    .select('id, business_id, location_id, name, price, duration_days, benefits, is_active, created_at')
...
export async function POST(req: Request) {
  const ip = getIp(req)
  if (!rateLimit(`memberships-create:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  ...
  const b = parsed.data
  if (b.type === undefined) {} // placeholder to satisfy TS
  const payload = {
    business_id: businessId,
    name: sanitize(b.name),
    price: b.price,
    duration_days: b.duration_days,
    benefits: b.benefits ?? { cuts: 4 },
    location_id: b.location_id || null,
    is_active: b.is_active ?? true,
  }

  const { data, error } = await supabase.from('memberships').insert(payload).select('id').single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
