...
async function handleTelegramStart(
  ...
  await supabase
    .from('businesses')
    .update({ telegram_chat_id: chatId } as Database['public']['Tables']['businesses']['Update'])
    .eq('id', businessId)
  await sendTelegramMessage(
    botToken,
    chatId,
...
async function handleTelegramHelp(
  botToken: string,
  chatId: string,
  text: string,
): Promise<boolean> {
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (await handleTelegramStart(supabase, biz, botToken, chatId, text, businessId, firstName))
      return NextResponse.json({ ok: true })
    if (await handleTelegramLink(supabase, botToken, chatId, text, businessId))
      return NextResponse.json({ ok: true })
    if (await handleTelegramToday(supabase, botToken, chatId, text, businessId))
      return NextResponse.json({ ok: true })
