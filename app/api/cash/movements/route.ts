...
export async function POST(req: Request) {
  ...
  const { data: register } = await supabase.from('cash_registers').select('id').eq('business_id', business.id).eq('status', 'open').maybeSingle()
  if (!register) return NextResponse.json({ error: 'no_open_register', message: 'Abre caja primero' }, { status: 404 })
