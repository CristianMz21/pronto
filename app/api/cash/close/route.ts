...
const BodySchema = z.object({ actual_cash: z.coerce.number().min(0).max(10_000_000), register_id: z.string().uuid().optional() })
...
export async function POST(req: Request) {
  ...
  // Find open register
  let q = supabase.from('cash_registers').select('id, opening_cash, opened_at').eq('business_id', business.id).eq('status', 'open')
  if (parsed.data.register_id) q = q.eq('id', parsed.data.register_id)
  const { data: register } = await q.maybeSingle()
  if (!register) return NextResponse.json({ error: 'no_open_register', message: 'No hay caja abierta' }, { status: 404 })

  // Calculate expected
  const { data: txs } = await supabase.from('transactions').select('amount').eq('business_id', business.id).eq('payment_method', 'cash').eq('status', 'completed').gte('created_at', register.opened_at)
  const { data: moves } = await supabase.from('cash_movements').select('type, amount').eq('register_id', register.id)
  ...
  }).eq('id', register.id).select('id, opening_cash, expected_cash, actual_cash, difference, status, opened_at, closed_at').single()
