...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const email = (app as { email: string; business_name: string; requested_plan?: string | null }).email
  const businessName = (app as { business_name: string }).business_name
  const requestedPlan = ((app as { requested_plan?: string | null }).requested_plan ?? 'starter') as string
  ...
  if (ownerId) {
    ...
    const baseSlug = businessName.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'barberia'
    let slug = baseSlug
    let attempt = 0
    while (true) {
    ...
    if (bizErr) {
      ...
      return NextResponse.json({ license_key: licenseKey, warning: bizErr.message }, { status: 201 })
    }
    if (biz) {
      try {
        const { insertOwnerAsEmployee } = await import('@/lib/create-business')
        await insertOwnerAsEmployee(admin as unknown as Parameters<typeof insertOwnerAsEmployee>[0], (biz as { id: string }).id, { id: ownerId, email } as unknown as Parameters<typeof insertOwnerAsEmployee>[1])
      } catch {}
