...
export async function GET() {
  ...
  const { data, error } = await supabase.from('service_combos').select('id, business_id, location_id, name, service_ids, price, duration_min, is_active, created_at').eq('business_id', businessId).order('created_at', { ascending: false })
...
export async function POST(req: Request) {
  const ip = getIp(req)
  if (!rateLimit(`service-combos-create:${ip}`, { limit: 60, windowMs: 10 * 60 * 1000 })) return NextResponse.json({ error: 'rate_limited' }, { status: 429 })
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthorized' }, { status: 401 })
  const businessId = await resolveBusinessId(supabase, user.id)
  ...
  const { data: services } = await supabase.from('services').select('id').eq('business_id', businessId).in('id', b.service_ids)
  const foundIds = new Set((services as { id: string }[] | null)?.map((s) => s.id) ?? [])
  const missing = b.service_ids.filter((id) => !foundIds.has(id))
  if (missing.length > 0) return NextResponse.json({ error: 'service_not_found', missing }, { status: 404 })
  ...
  const { data, error } = await supabase.from('service_combos').insert(payload).select('id').single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
