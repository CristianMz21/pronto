  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-md mx-auto">
      ...
      <form action={registerClient} className="space-y-4">
        ...
        <PasswordInput
          id="password" name="password" label={t('passwordLabel')} placeholder={t('passwordPlaceholder')} required minLength={8} autoComplete="new-password"
        />
        <button
          type="submit"
      ...
      <p className="text-sm text-gray-500 text-center mt-4">
        ¿Ya tenés cuenta? <Link href={`/client/login?redirect=${encodeURIComponent(redirectParam)}`} className="text-blue-600 hover:underline">Iniciar sesión</Link>
      </p>
    </div>
  )
}
