      ...
      {services.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center text-gray-500">
            Sin servicios aún. Crea el primero.
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {services.map((s) => (
            <Card
              key={s.id}
              className={`${!s.is_active ? 'opacity-50' : ''} hover:shadow-md transition-shadow`}
            >
              <CardContent className="p-4">
                ...
                {s.description && (
                  <p className="mt-2 text-xs text-gray-600 line-clamp-2">{s.description}</p>
                )}
                <div className="mt-3 flex items-center justify-between">
                  <span className="font-bold text-sm">
                    {formatCurrency(Number(s.price), 'COP')}
                  </span>
                  {s.cost && (
                    <span className="text-xs text-gray-400">
                      Costo {formatCurrency(Number(s.cost), 'COP')}
                    </span>
                  )}
                </div>
