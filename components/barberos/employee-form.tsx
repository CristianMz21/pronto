...
export function EmployeeForm({
  ...
  async function toggleService(serviceId: string) {
    ...
    const res = await fetch('/api/employee-services', {
      ...
      body: JSON.stringify({ employee_id: employee.id, service_id: serviceId, action: isAssigned ? 'unassign' : 'assign' }),
    })
    if (!res.ok) {
      setError('Error al asignar servicio')
      return
    }
    setAssigned((prev) => (isAssigned ? prev.filter((id) => id !== serviceId) : [...prev, serviceId]))
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 overflow-auto">
      <Card className="w-full max-w-lg max-h-[90vh] overflow-auto">
        <CardHeader>
          <CardTitle className="text-base">{employee ? 'Editar barbero' : 'Nuevo barbero'}</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="text-xs text-gray-500">Nombre *</label>
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="w-full mt-1 border rounded-lg px-3 py-2 text-sm" placeholder="Andrés" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500">Teléfono</label>
                <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm" placeholder="3001234567" />
              </div>
              <div>
                <label className="text-xs text-gray-500">Email</label>
                <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm" placeholder="a@escuderia.com" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
              ...
              <div>
                ...
                <input type="color" value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value })} className="w-full mt-1 border rounded-lg h-9 p-1" />
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-500">Especialidades (coma separadas)</label>
              <input value={form.specialties} onChange={(e) => setForm({ ...form, specialties: e.target.value })} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm" placeholder="corte, barba, color" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500">Comisión %</label>
                <input type="number" min={0} max={100} step={0.5} value={form.commission_rate} onChange={(e) => setForm({ ...form, commission_rate: e.target.value })} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm" placeholder="50" />
              </div>
              <div>
                <label className="text-xs text-gray-500">Comisión fija (COP)</label>
                <input type="number" min={0} value={form.commission_fixed} onChange={(e) => setForm({ ...form, commission_fixed: e.target.value })} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm" placeholder="15000" />
              </div>
            </div>
            {locations && locations.length > 0 && (
              <div>
                <label className="text-xs text-gray-500">Sede</label>
                <select value={form.location_id} onChange={(e) => setForm({ ...form, location_id: e.target.value })} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm">
            ...
            <div>
              ...
              <div className="mt-1 border rounded-lg p-3 max-h-32 overflow-auto space-y-1 bg-gray-50">
                {services.length === 0 ? <p className="text-xs text-gray-400">No hay servicios — crea primero en /servicios</p> : services.map((s) => (
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button type="submit" disabled={saving || !form.name} className="flex-1">{saving ? 'Guardando...' : employee ? 'Guardar' : 'Crear'}</Button>
            </div>
          </form>
        </CardContent>
      </Card>
