...
async function main() {
  ...
  doc
    .fillColor(accent)
    .fontSize(8)
    .text('PRONTO BARBER • 43/43 tasks • 7.863 citas', 400, 64, { align: 'right', width: 150 })
  ...
  kpis.forEach((k, i) => {
    ...
    doc
      ...
      .fontSize(7)
      .font('Helvetica')
      .text(k[0].toUpperCase(), x, yy, { width: colW })
    doc
      .fillColor(brand)
      .fontSize(11)
      .font('Helvetica-Bold')
      .text(k[1], x, yy + 10, { width: colW })
  })
  ...
  y = table(
    'Evolución Mensual (citas completadas)',
    ['Mes', 'Citas', 'Revenue'],
    mensualRows,
    [120, 130, 265],
    y,
  ...
  // Barberos
  const barberRows = perBarber.map((b) => [
    b.name,
    fmt(b.citas),
    fmt(b.ventas),
    `$${fmt(b.revenue)}`,
  ...
  // Top clientes
  const cliRows = topClients
    .slice(0, 8)
    .map((c) => [c.name, c.phone, fmt(c.citas), `$${fmt(c.gasto)}`])
  y = table(
    'Top Clientes (por gasto)',
    ['Cliente', 'Tel', 'Citas', 'Gasto'],
    cliRows,
    [160, 130, 60, 165],
    y,
  )
