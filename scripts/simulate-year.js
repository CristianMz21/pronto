...
const FIRST_NAMES = ['Carlos','Juan','Luis','Miguel','Jorge','Andrés','Felipe','Santiago','Daniel','Alejandro','Camilo','Sebastián','Mateo','Nicolás','David','Julián','Fernando','Ricardo','Óscar','Hernán','Ana','María','Laura','Sofia','Valentina','Camila','Daniela','Paola','Carolina','Diana','Lucía','Andrea','Marcela','Natalia','Claudia']
const LAST_NAMES = ['García','Rodríguez','Martínez','López','González','Pérez','Sánchez','Ramírez','Torres','Flores','Rivera','Gómez','Díaz','Reyes','Morales','Cruz','Ortiz','Gutiérrez','Mendoza','Vargas','Castro','Silva','Herrera','Peña','Álvarez','Jiménez','Ruiz','Aguilar','Castillo','Moreno']
const TAGS_POOL = [['frecuente'],['vip'],['nuevo'],['frecuente','vip'],[],[],[],['inactivo'],['frecuente'],['barba']]
const NOTES_POOL = ['', '', '', 'Prefiere corte con tijera', 'Cliente exigente', 'Siempre con barba', 'Viene cada 15 días', 'Paga en efectivo', '']

function randomPhone() {
  const prefix = [300,301,310,311,312,313,314,315,316,320,321,322][Math.floor(Math.random()*12)]
...
async function main() {
  const client = new Client({ connectionString: DB_URL, ssl: SSL ? { rejectUnauthorized: false } : false })
  ...
  const { rows: employees } = await client.query(`select id, name, specialties, commission_rate from employees where business_id=$1 and is_active=true`, [BUSINESS_ID])
  const { rows: services } = await client.query(`select id, name, price, duration_min, category from services where business_id=$1 and is_active=true`, [BUSINESS_ID])
  const { rows: empServices } = await client.query(`select employee_id, service_id from employee_services where employee_id = any($1)`, [employees.map(e=>e.id)])
  ...
  for (let i=0;i<120;i++) {
    ...
    const email = Math.random()<0.4 ? `${name.toLowerCase().replace(' ','.')}${i}@gmail.com` : null
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate()+1)) {
    ...
    const basePerBarber = dow === 6 ? 7 + Math.floor(Math.random()*3) : 5 + Math.floor(Math.random()*4) // Sat 7-9, Mon-Fri 5-8
    ...
    for (const appt of batch) {
      try {
        const { rows: apptRows } = await client.query(
          ...
          [appt.business_id, appt.client_id, appt.employee_id, appt.service_id, appt.starts_at, appt.ends_at, appt.status, appt.price, appt.source]
        )
        ...
        if (appt.status === 'completed') {
          ...
          const items = JSON.stringify([{ service_id: appt.service_id, name: services.find(s=>s.id===appt.service_id).name, price: appt.price, qty: 1 }])
          await client.query(
        ...
        if (e.message && e.message.includes('slot_already_booked')) {
          skippedDouble++
        } else if (e.message && (e.message.includes('barber_unavailable') || e.message.includes('barber_not_qualified') || e.message.includes('outside_availability'))) {
  ...
  console.log(`Inserted ${insertedAppts} appointments (skipped ${skippedDouble} double/unavailable)`)
  console.log(`Created ${totalTransactions} transactions`)

  // Generate a few inventory items and movements for simulation
  console.log('Seeding inventory...')
  const inventory = [
    { name: 'Pomada Escudería', sku: 'POM-001', category: 'pomadas', quantity: 12, sell_price: 45000, cost_price: 20000 },
    { name: 'Cera Matte', sku: 'CER-002', category: 'ceras', quantity: 3, sell_price: 38000, cost_price: 18000 },
    { name: 'Shampoo Barba', sku: 'SHA-003', category: 'shampoo', quantity: 2, sell_price: 28000, cost_price: 12000 },
    { name: 'Aceite para Barba', sku: 'ACE-004', category: 'aceites', quantity: 8, sell_price: 32000, cost_price: 15000 },
    { name: 'Navajas Desechables x100', sku: 'NAV-005', category: 'insumos', quantity: 1, sell_price: 15000, cost_price: 7000 },
  ]
  for (const item of inventory) {
    await client.query(
      ...
      [BUSINESS_ID, item.name, item.sku, item.category, item.quantity, item.sell_price, item.cost_price]
    )
  ...
  const now = new Date()
  for (let m=0; m<6; m++) {
    const opened = new Date(now); opened.setMonth(now.getMonth()-m); opened.setDate(1); opened.setHours(8,0,0,0)
    const closed = new Date(opened); closed.setDate(2); closed.setHours(19,0,0,0)
    ...
    await client.query(
      ...
      [BUSINESS_ID, (await client.query(`select owner_id from businesses where id=$1`, [BUSINESS_ID])).rows[0].owner_id, opening, expected, actual, opened.toISOString(), closed.toISOString()]
    )
  }
  ...
  perBarber.forEach(r => console.log(`  ${r.name}: ${r.appts} citas, ${r.sales} ventas, $${Number(r.revenue).toLocaleString('es-CO')} revenue, $${Number(r.commissions).toLocaleString('es-CO')} comisión`))
...
function wallclockToUtc(date, time, tz) {
  ...
  const parts = new Intl.DateTimeFormat('en-US', { timeZone: tz, year:'numeric', month:'numeric', day:'numeric', hour:'numeric', minute:'numeric', second:'numeric', hour12:false }).formatToParts(noonUtc)
