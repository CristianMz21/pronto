...
async function main() {
  ...
  // Sheet 2: Mensual
  const mensualHeader = ['Mes','Citas','Completadas','Cancel','No-show','Revenue citas','Ventas','Revenue','Cash','Card','Transfer']
  const mensualData = monthly.map(m => {
    const tx = monthlyTx.find(x=>x.mes===m.mes) || {}
    return [m.mes, Number(m.total_citas), Number(m.citas_ok), Number(m.cancel), Number(m.noshow), Number(m.revenue_citas), Number(tx.ventas||0), Number(tx.revenue||0), Number(tx.cash||0), Number(tx.card||0), Number(tx.transf||0)]
  })
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([mensualHeader, ...mensualData]), 'Mensual')

  // Sheet 3: Barberos
  const barberHeader = ['Barbero','Citas','Ventas','Revenue','Comisión','Cancel','No-show','Comisión %']
  const barberData = perBarber.map(b => [b.name, Number(b.citas), Number(b.ventas), Number(b.revenue), Number(b.comision), Number(b.cancel), Number(b.noshow), b.revenue>0 ? (Number(b.comision)/Number(b.revenue)*100).toFixed(1)+'%' : '0%'])
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([barberHeader, ...barberData]), 'Barberos')

  // Sheet 4: Servicios
  const svcHeader = ['Servicio','Categoría','Citas','Revenue','Ticket']
  const svcData = perService.map(s => [s.name, s.category, Number(s.citas), Number(s.revenue), Number(s.ticket||0).toFixed(0)])
  ...
  const cliData = topClients.map(c => [c.name, c.phone, (c.tags||[]).join(', '), Number(c.citas), Number(c.gasto), c.ultima ? new Date(c.ultima).toISOString().slice(0,10) : ''])
...
| Cliente | Citas | Gasto |
|---------|-------|-------|
${topClients.slice(0,5).map(c=>`| ${c.name} | ${c.citas} | $${Number(c.gasto).toLocaleString('es-CO')} |`).join('\n')}
`
