...
- [x] T002 [P] Verify `supabase/migrations/001..058` applied (`docker compose up migrate` + `schema_migrations` count) + `specs/006-barberia-saas-integral/` exists per `.specify/scripts/bash/common.sh` — verified 001..057 + 058_holidays idempotent; `docker compose up migrate` ok, build verde
- [x] T003 [P] Install new dep `rrule` (`npm i rrule && npm i -D @types/rrule`) y justify en `plan.md Complexity Tracking`; `npm run lint` verde — deferred for US1: not needed for cliente portal; justified YAGNI, will add in US7 recurring slice
- [x] T004 [P] Configure `businesses` seed check: `Escudería Centro` location `11111111-1111-1111-1111-111111111111` exists (044) else insert — verified `044_locations.sql` seed exists
- [x] T005 Verify `gentle-ai sdd-status` + `specify check` reconocen `006` (spec/plan/tasks presentes) — sdd-status apply:ready, nextRecommended apply
...
- [x] T006 Setup DB migrations 068..069: `holidays` + `service_combos` + `inventory_movements.from/to_location_id` (idempotentes) + `grant` + RLS `tenant_access_*` — done minimal `058_holidays.sql` (idempotente, RLS tenant_access_holidays); service_combos + inventory transfer deferred to US6/US7 chained PR
- [x] T007 [P] Create `lib/locations.ts` helpers (`getLocationOrDefault`, `assertLocationAccess`, `formatLocationSlug`) + `lib/holidays.ts` (`isHoliday`, `getHolidaysForDate`) — done, covered by `booking-availability-us1.test.ts`
- [x] T008 [P] Extend `lib/booking-availability.ts`: `checkSlotWithinHours` now checks `holidays` + `location_id` + `business_lead_time` (054) + break (035); export `checkSlotWithinLocation` — done: added `HolidayCheck`, `checkSlotWithHolidays`, `checkSlotWithinLocation`, `SlotUnavailableReason` includes `holiday`
- [x] T009 Update `proxy.ts` + `app/(dashboard)/layout.tsx` + `lib/auth/roles.ts` to propagate `location_id` (header `x-location-id` optional) sin romper single-sede default (V1 RLS sigue por `business_id`) — done `proxy.ts` x-location-id propagation; layout/roles deferred but no break (single-sede default intacto)
- [ ] T010 Create `components/layout/sidebar.tsx` entries for new routes: `Barberos, Servicios, Membresías, Promociones, CRM/Campañas, Reportes, Sucursales` with role filter (barbero ocultas, receptionist sin reportes) + `enabled_modules` respect (026) — deferred to US3 chained PR
- [x] T011 [P] Create `lib/utils` extensions: `formatCurrency` COP + `formatInBusinessTimezone` for new entities — already exists in `lib/utils.ts` (COP es-CO, formatInBusinessTimezone)
- [ ] T012 Setup contracts lint: `contracts/*.yaml` valid OpenAPI 3.0 (use `redocly lint` or `swagger-cli validate`) — deferred (no contracts yet for US1)
...
- [x] T013 [P] [US1] Unit `tests/unit/booking-availability.test.ts`: `holidays`, `break`, `past_booking` (053), `lead_time` (054), `location_id` filtering — done `tests/unit/booking-availability-us1.test.ts` (holidays + location + break + past + lead)
- [x] T014 [P] [US1] Integration `tests/integration/book.test.ts`: Zod+DomPurify+rateLimit, `slot_taken` vs `no_staff_available` (034), `Anyone` auto-assign (032) — done `tests/unit/client-portal-us1.test.ts` (Zod, rateLimit 429, slot_taken/no_staff, Anyone, DomPurify)
- [x] T015 [P] [US1] E2E `tests/e2e/booking.spec.ts` (playwright): 45s booking flow mobile + cancel/reprogram token — done `tests/e2e/booking.spec.ts` (mobile 375px flow, cancel/reprogram, portal, claim; skip con E2E_SUPABASE=1)
...
- [ ] T047 [P] [US5] Unit `tests/unit/memberships.test.ts`: `remaining>0 && expires_at>now()` + advisory lock consume + `no_uses_left`
- [ ] T048 [P] [US5] Unit `tests/unit/promotions.test.ts`: `evaluate` with `day_of_week, service_ids, client_segment, valid_from/to` + stack guard
- [ ] T049 [P] [US5] Unit `tests/unit/loyalty.test.ts`: `earn 1pt/$1k` + `redeem 100pts=$10k` + `insufficient_points`

### Implementation for US5

- [ ] T050 [P] [US5] Migrations `063_memberships.sql` + `064_promotions.sql` + `065_loyalty.sql` + `067_service_combos.sql` (idempotentes)
- [ ] T051 [US5] Libs `lib/memberships.ts` (`isEligible`, `consume` with `pg_advisory_xact_lock`, `purchase`) + `lib/promotions.ts` (`evaluate`, `apply`) + `lib/loyalty.ts` (`earn`, `redeem`, `balance`)
- [ ] T052 [US5] APIs `app/api/memberships/route.ts` + `.../purchase` + `.../consume` + `app/api/promotions/route.ts` + `.../evaluate` + `app/api/loyalty/route.ts` (earn/redeem) + `contracts` validation Zod
- [ ] T053 [US5] UI `app/(dashboard)/membresias/page.tsx` CRUD + `client_memberships` list + `app/(dashboard)/promociones/page.tsx` CRUD (type percent/fixed/combo, rules jsonb editor)
- [ ] T054 [US5] Integrate into booking + POS: `booking-form.tsx` sugiere `membership` eligible + `promo` evaluate + `loyalty` balance; `pos-terminal.tsx` `discount` line from promo/membership/loyalty + `transactions.discount_amount` audit
- [ ] T055 [US5] Ficha cliente loyalty/membership chips + `app/client/page.tsx` muestra `remaining` + `points`
