...
begin
  if (select count(*) from public.inventory_items where business_id=v_bid) < 80 then
...
declare v_bid uuid := '17c1a2b5-5d3b-4d84-bbb1-d361077d4c95';
begin
  if (select count(*) from public.loyalty_accounts where business_id=v_bid) < 500 then
...
begin
  if (select count(*) from public.client_memberships where business_id=v_bid) < 100 then
...
begin
  if (select count(*) from public.recurring_appointments where business_id=v_bid) < 50 then
