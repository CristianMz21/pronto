...
export function createSupabaseChain(result: { data: any; error: any } = { data: null, error: null }) {
  const chain: any = {}
...
export function mockSupabaseClient(overrides: Record<string, any> = {}) {
  ...
  return {
    from: () => defaultChain,
    auth: { getUser: async () => ({ data: { user: null }, error: null }), signInWithPassword: async () => ({ data: {}, error: null }), signUp: async () => ({ data: {}, error: null }) },
    storage: { from: () => ({ upload: async () => ({ data: {}, error: null }), getPublicUrl: () => ({ data: { publicUrl: 'http://example.com/img.jpg' } }) }) },
    ...overrides,
  }
}
