...
function makeChain(result: any) {
  ...
  methods.forEach(m => { c[m] = vi.fn((...a: any[]) => c) })
  return c
}
...
function setupBook(opts: BookMockOpts = {}) {
  ...
  const biz = opts.biz !== undefined ? opts.biz : { timezone: 'UTC', min_advance_minutes: 30, booking_lead_time_enabled: true }
  ...
  const clientInsert = opts.clientInsert !== undefined ? opts.clientInsert : { data: { id: 'client-new' }, error: null }
  const appt = opts.appointment !== undefined ? opts.appointment : { data: { id: 'appt-1' }, error: null }
  ...
  mockClient._chains = { serviceChain, bizChain, bhChain, clientsSelectChain, clientInsertChain: clientsInsertChain, apptChain }
...
describe('api/book integration — configurable lead time (054)', () => {
  ...
  it('in_past: booking wall time already passed => 400 in_past', async () => {
    ...
    setupBook({ businessHours: allOpen, biz: { timezone: 'UTC', min_advance_minutes: 30, booking_lead_time_enabled: true } })
  ...
  it('too_soon with lead 15: 10 min ahead => too_soon, 15 min ahead => ok', async () => {
    ...
    const allOpen = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }))
    // lead 15
    setupBook({ businessHours: allOpen, biz: { timezone: 'UTC', min_advance_minutes: 15, booking_lead_time_enabled: true } })
  ...
  it('too_soon with lead 60: 30 min ahead => too_soon, 60 min ahead => ok', async () => {
    ...
    vi.setSystemTime(new Date('2030-01-15T10:00:00.000Z'))
    const allOpen = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }))
    setupBook({ businessHours: allOpen, biz: { timezone: 'UTC', min_advance_minutes: 60, booking_lead_time_enabled: true } })
  ...
  it('lead disabled: even 5 min ahead allowed (only past blocked)', async () => {
    ...
    vi.setSystemTime(new Date('2030-01-15T10:00:00.000Z'))
    const allOpen = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }))
    setupBook({ businessHours: allOpen, biz: { timezone: 'UTC', min_advance_minutes: 60, booking_lead_time_enabled: false } })
  ...
  it('break time => 400 break', async () => {
    const withBreak = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: '12:00', break_end: '13:00' }))
  ...
  it('éxito: valid future booking with custom lead 15 passes', async () => {
    ...
    const allOpen = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }))
    setupBook({ businessHours: allOpen, biz: { timezone: 'UTC', min_advance_minutes: 15, booking_lead_time_enabled: true }, clientsMatches: [] })
    const res = await POST(req(payload({ date: '2099-06-15', time: '10:00' })) as any)
    expect(res.status).toBe(200)
    const j = await res.json()
  ...
  it('lead 0 with enabled true allows immediate future (only past blocked)', async () => {
    ...
    vi.setSystemTime(new Date('2030-01-15T10:00:00.000Z'))
    const allOpen = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }))
    setupBook({ businessHours: allOpen, biz: { timezone: 'UTC', min_advance_minutes: 0, booking_lead_time_enabled: true } })
  ...
  it('timezone: Bogota lead 30 respects wall time conversion', async () => {
    ...
    const allOpen = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }))
    setupBook({ businessHours: allOpen, biz: { timezone: 'America/Bogota', min_advance_minutes: 30, booking_lead_time_enabled: true } })
