...
function setupBookMocks(opts: BookMockOpts = {}) {
  ...
  const clientInsertRes =
    opts.clientInsert !== undefined
      ? opts.clientInsert
      : { data: { id: 'new-client-id' }, error: null }
  const apptRes =
    opts.appointment !== undefined ? opts.appointment : { data: { id: 'appt-id' }, error: null }
...
function setupModulesMocks(opts: { user?: any | null; updateError?: any | null } = {}) {
  const user = opts.user !== undefined ? opts.user : { id: 'user-1' }
...
describe('api-book-health-strict', () => {
  beforeEach(() => {
    ...
    global.fetch = vi
      .fn()
      .mockResolvedValue({ ok: true, text: async () => '', json: async () => ({}) } as any)
  ...
  describe('/api/book POST', () => {
    ...
    describe('validation_failed 422 per field', () => {
      ...
      it('email invalid', async () => {
        const res = await BookPOST(
          bookRequest(validBookPayload({ email: 'not-email', phone: null })) as any,
        )
    ...
    it('contact_required 400 when no phone nor email', async () => {
      ...
      const res = await BookPOST(
        bookRequest({ ...payload, phone: undefined, email: undefined }) as any,
      )
    ...
    describe('outside_availability', () => {
      it('closed reason', async () => {
        ...
        const res = await BookPOST(
          bookRequest(validBookPayload({ date: '2099-06-15', time: '10:00' })) as any,
        )
        expect(res.status).toBe(400)
        const j = await res.json()
      ...
      it('outside_hours reason', async () => {
        ...
        const res = await BookPOST(
          bookRequest(validBookPayload({ date: '2099-06-15', time: '08:00' })) as any,
        )
        expect(res.status).toBe(400)
        const j = await res.json()
      ...
      it('outside_hours when ends after close', async () => {
        ...
        setupBookMocks({ businessHours: allOpen })
        const res = await BookPOST(
          bookRequest(validBookPayload({ date: '2099-06-15', time: '19:45' })) as any,
        ) // 19:45+30 => 20:15 >20:00
        expect(res.status).toBe(400)
        expect((await res.json()).reason).toBe('outside_hours')
      })
      it('break reason', async () => {
        const allWithBreak = [0, 1, 2, 3, 4, 5, 6].map((d) => ({
          day_of_week: d,
          is_open: true,
          open_time: '09:00',
          close_time: '20:00',
        ...
        const res = await BookPOST(
          bookRequest(validBookPayload({ date: '2099-06-15', time: '12:30' })) as any,
        )
        expect(res.status).toBe(400)
    ...
    it('client upsert existing found -> update name/email when cambian', async () => {
      ...
      expect(chains.clientsUpdateChain.update).toHaveBeenCalledWith(
    ...
    it('client creation fail 500', async () => {
      ...
      const res = await BookPOST(
        bookRequest(
          validBookPayload({ date: '2099-06-15', time: '10:00', phone: '+123', email: 'a@b.com' }),
        ) as any,
      )
    ...
    it('sanitize name strips tags', async () => {
      ...
      expect(chains.clientsInsertChain.insert).toHaveBeenCalledWith(
        expect.objectContaining({ name: 'John alert(1)' }),
      )
    })
    ...
    describe('appt insert triggers', () => {
      const futurePayload = () =>
        validBookPayload({ date: '2099-06-15', time: '10:00', phone: '+123' })
      const baseHours = [0, 1, 2, 3, 4, 5, 6].map((d) => ({
        day_of_week: d,
        is_open: true,
        open_time: '09:00',
        close_time: '20:00',
      ...
      it('no_staff_available 409', async () => {
        setupBookMocks({
          ...
          clientsMatches: [],
          appointment: { data: null, error: { message: 'no_staff_available' } },
        })
      ...
      it('slot_already_booked 409 -> slot_taken', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'slot_already_booked' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(409)
      ...
      it('barber_not_qualified 400', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'barber_not_qualified' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
      ...
      it('barber_unavailable 409', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'barber_unavailable' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(409)
      ...
      it('barber_inactive 400', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'barber_inactive' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
      ...
      it('outside_availability from DB closed', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'outside_availability closed' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
        const j = await res.json()
      ...
      it('outside_availability from DB break', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'outside_availability break' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
        expect((await res.json()).reason).toBe('break')
      ...
      it('outside_availability from DB outside_hours (default)', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'outside_availability outside_hours' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
        expect((await res.json()).reason).toBe('outside_hours')
      ...
      it('outside_availability fallback to outside_hours when no detail', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'outside_availability' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
        expect((await res.json()).reason).toBe('outside_hours')
      ...
      it('in_past from DB 400', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'in_past' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
        expect((await res.json()).error).toBe('in_past')
      ...
      it('too_soon from DB 400', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'too_soon' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(400)
        expect((await res.json()).error).toBe('too_soon')
      ...
      it('booking_failed 500 generic', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: { message: 'some unknown error' } },
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(500)
      ...
      it('booking_failed when appt null no error', async () => {
        setupBookMocks({
          businessHours: baseHours,
          clientsMatches: [],
          appointment: { data: null, error: null } as any,
        })
        const res = await BookPOST(bookRequest(futurePayload()) as any)
        expect(res.status).toBe(500)
        expect((await res.json()).error).toBe('booking_failed')
    ...
    it('timezone fallback to UTC when biz is null', async () => {
      ...
      global.fetch = fetchSpy as any
      const res = await BookPOST(
        bookRequest(validBookPayload({ date: '2099-06-15', time: '10:00', phone: '+123' })) as any,
      )
      expect(res.status).toBe(200)
  ...
  describe('/api/check-slug GET', () => {
    ...
    it('ownBusiness exclude: neq called when ownBusiness exists', async () => {
      const { chains } = setupCheckSlugMocks({
        user: { id: 'u1' },
        ownBusiness: { id: 'own-id' },
        count: 0,
      })
  ...
  describe('/api/business/modules PATCH', () => {
    ...
    it('fast-check modules filter property', async () => {
      await fc.assert(
        fc.asyncProperty(fc.array(fc.string()), async (arr) => {
          const { chains } = setupModulesMocks({ user: { id: 'u1' } })
          const req = new NextRequest('http://localhost/api/business/modules', {
            method: 'PATCH',
