...
describe('zero coverage files', () => {
  ...
  it('client appointments PATCH covers all branches', async () => {
    ...
    req = new NextRequest('http://localhost/api/client/appointments/a1', {
      method: 'PATCH',
      body: JSON.stringify({ action: 'cancel' }),
    } as any)
  ...
  it('client appointments PUT covers validation and reschedule', async () => {
    ...
    let req = new NextRequest('http://localhost/api/client/appointments/a1', {
      method: 'PUT',
      body: JSON.stringify({ date: '2026-12-01', time: '10:00' }),
    } as any)
    ...
    req = new NextRequest('http://localhost/api/client/appointments/a1', {
      method: 'PUT',
      body: JSON.stringify({ date: 'bad', time: 'bad' }),
    } as any)
    res = await PUT(req as any, { params: Promise.resolve({ id: 'a1' }) })
    ...
    req = new NextRequest('http://localhost/api/client/appointments/a1', {
      method: 'PUT',
      body: JSON.stringify({ date: futureDate, time: '10:00' }),
    } as any)
    res = await PUT(req as any, { params: Promise.resolve({ id: 'a1' }) })
  ...
  it('covers client loginClient and registerClient actions', async () => {
    ...
    fd.set('redirect', '/client/dashboard')
    ...
    fd2.set('email', 'a@b.com')
    fd2.set('password', 'wrong')
    fd2.set('redirect', '/client/dashboard')
    ...
    vi.mocked(createClient).mockResolvedValue({
      auth: { signUp: vi.fn(async () => ({ data: {}, error: null })) },
    } as any)
    const fd3 = new FormData()
    fd3.set('email', 'a@b.com')
    fd3.set('password', 'pass123')
    fd3.set('name', '')
    fd3.set('phone', '+1')
    ...
    fd4.set('email', 'b@b.com')
    fd4.set('password', 'pass123')
    fd4.set('name', 'John')
    fd4.set('phone', '+1')
    ...
    fd5.set('email', 'c@b.com')
    fd5.set('password', 'pass123')
    fd5.set('name', 'John')
  ...
  it('covers low-stock forbidden and threshold cases', async () => {
    ...
    const req = new NextRequest('http://localhost/api/email/low-stock', {
      method: 'POST',
      body: JSON.stringify({ itemId: 'unknown' }),
    } as any)
