...
describe('viber ultra strict 100', ()=>{
  ...
  it('covers viber webhook 77 client not found', async()=>{
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:'tc'}, error:null})) })) })) } as any
        if(t==='clients') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:null, error:null})) })) })) })), update: vi.fn(()=>({ eq: vi.fn(async()=>({error:null})) })) } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    const req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'conversation_started', context:'client_11111111-1111-1111-1111-111111111111', user:{id:'u1', name:'John'}})} as any)
    const res=await POST(req as any)
    expect(res.status).toBe(200)
    const viber=await import('@/lib/viber')
    expect(viber.sendViberMessage).toHaveBeenCalledWith('tok','u1', expect.stringContaining('Link not found'))
  })
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='businesses') return {
          select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:null}, error:null})) })) })),
          update: vi.fn(()=>({ eq: vi.fn(async()=>({error:null})) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    const req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'conversation_started', user:{id:'u1', name:'John'}})} as any)
    const res=await POST(req as any)
    expect(res.status).toBe(200)
    const viber=await import('@/lib/viber')
    expect(viber.sendViberMessage).toHaveBeenCalledWith('tok','u1', expect.stringContaining('You are now connected'))
  })
  ...
  it('covers viber webhook 126-131 /start', async()=>{
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='businesses') return {
          select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:'tc'}, error:null})) })) })),
          update: vi.fn(()=>({ eq: vi.fn(async()=>({error:null})) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    const req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'message', sender:{id:'u1'}, message:{text:'/start'}})} as any)
    const res=await POST(req as any)
    expect(res.status).toBe(200)
    const viber=await import('@/lib/viber')
    expect(viber.sendViberMessage).toHaveBeenCalledWith('tok','u1', expect.stringContaining('Connected to'))
  })
  ...
  it('covers viber webhook 182 no appointments today', async()=>{
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:'u1'}, error:null})) })) })) } as any
        if(t==='appointments') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ gte: vi.fn(()=>({ lte: vi.fn(()=>({ order: vi.fn(async()=>({data:[], error:null})) })) })) })) })) } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    const req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'message', sender:{id:'u1'}, message:{text:'/today'}})} as any)
    const res=await POST(req as any)
    expect(res.status).toBe(200)
    const viber=await import('@/lib/viber')
  ...
  it('covers remaining 107 for client appointments and 37 for viber set-webhook', async()=>{
    ...
    vi.mocked(createServiceClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='appointments') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:{id:'a1', client_id:'c1', starts_at:new Date(Date.now()+48*3600*1000).toISOString(), status:'confirmed', business_id:'b1', service_id:'s1', services:{duration_min:30}}, error:null})) })) })) } as any
    ...
    let req=new NextRequest('http://localhost/api/client/appointments/a1', { method:'PUT', body: JSON.stringify({date: pastDate, time:'10:00'})} as any)
    ...
    vi.mocked(createClient).mockResolvedValue({
      auth:{ exchangeCodeForSession: vi.fn(async()=>({error:{message:'bad'}})), getUser: vi.fn(async()=>({data:{user:null}, error:null})) },
  ...
  it('adversarial: fuzz viber webhook with malicious inputs', async()=>{
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:null}, error:null})) })) })), update: vi.fn(()=>({ eq: vi.fn(async()=>({error:null})) })) } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    const malicious="<script>alert(1)</script>"
    const req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'conversation_started', user:{id:'u1', name: malicious}})} as any)
    const res=await POST(req as any)
    expect(res.status).toBe(200)
    const viber=await import('@/lib/viber')
