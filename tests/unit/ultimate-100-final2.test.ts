...
vi.mock('serwist', () => ({
  Serwist: class {
    ...
    constructor(_opts: any) {
      _opts.fallbacks.entries[0].matcher({ request: { destination: 'document' } })
      _opts.fallbacks.entries[0].matcher({ request: { destination: 'image' } })
    }
  },
  NetworkFirst: vi.fn(),
...
describe('ultimate final2 100', () => {
        ...
        if (table === 'clients') {
          ...
          if (clientCall === 1) {
            ...
            c.select = vi.fn(() => ({
              not: vi.fn(() => ({
                is: vi.fn(async () => ({
                  data: [bdayWithAll, bdayNoContact, bdayNonString],
                  error: null,
                })),
              })),
            }))
  ...
  it('email confirm all channels with walk-in and no biz', async () => {
    ...
    ;(supa.createClient as any).mockReturnValue({
      from: vi.fn((table: string) => {
        if (table === 'appointments')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: apptWalkIn, error: null })) })),
            })),
          } as any
        if (table === 'businesses')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: bizNull, error: null })) })),
            })),
          } as any
        if (table === 'notification_log')
          return {
    ...
    const req = new NextRequest('http://localhost/api/email/confirm', {
      ...
      body: JSON.stringify({ appointmentId: 'a1', formEmail: 'walkin@test.com' }),
    } as any)
  ...
  it('email low-stock covers all branches including owner fallback', async () => {
    ...
    ;(serverMod.createClient as any).mockResolvedValue({
    ...
    ;(supaMod.createClient as any).mockReturnValue({
      from: vi.fn((table: string) => {
        if (table === 'inventory_items')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: itemLow, error: null })) })),
            })),
          } as any
        if (table === 'businesses') {
          let call = 0
    ...
    ;(supaMod.createClient as any).mockReturnValue({
      from: vi.fn((table: string) => {
        if (table === 'inventory_items')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: itemHigh, error: null })) })),
            })),
          } as any
        if (table === 'businesses')
          return {
  ...
  it('viber webhook covers all branches', async () => {
    ...
    const cases = [
      ...
      {
        body: { event: 'conversation_started', context: 'client_invalid', sender: { id: 'u1' } },
        clients: null,
      },
      {
        body: { event: 'conversation_started', context: '', sender: { id: 'newUser' } },
        clients: null,
        biz: { ...biz, viber_chat_id: null },
      },
      {
        body: { event: 'message', message: { text: '/start' }, sender: { id: 'u1' } },
        clients: null,
      },
      {
        body: { event: 'message', message: { text: '/link' }, sender: { id: 'u1' } },
        clients: null,
      },
      {
        body: { event: 'message', message: { text: '/link 123' }, sender: { id: 'u1' } },
        clients: { id: 'c1', name: 'Client' },
      },
      {
        body: { event: 'message', message: { text: '/link 999' }, sender: { id: 'u1' } },
        clients: null,
      },
      {
        body: { event: 'message', message: { text: '/today' }, sender: { id: 'user1' } },
        clients: null,
        appts: [
          {
      ...
      {
        body: { event: 'message', message: { text: '/today' }, sender: { id: 'other' } },
        clients: null,
        appts: [],
      },
      {
        body: { event: 'message', message: { text: '/help' }, sender: { id: 'u1' } },
        clients: null,
      },
      {
        body: { event: 'message', message: { text: 'unknown' }, sender: { id: 'u1' } },
        clients: null,
      },
      ...
      vi.mocked(createClient).mockReturnValue({
        from: vi.fn((table: string) => {
          if (table === 'businesses')
            return {
              select: vi.fn(() => ({
                eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: testBiz, error: null })) })),
              })),
            } as any
            ...
            return {
              select: vi.fn(() => ({
                eq: vi.fn(() => ({
                  gte: vi.fn(() => ({
                    lte: vi.fn(() => ({
                      order: vi.fn(async () => ({ data: testAppts || [], error: null })),
                    })),
                  })),
                })),
              })),
    ...
    // no bid
    const { POST } = await import('@/app/api/viber/webhook/route')
    const reqNoBid = new NextRequest('http://localhost/api/viber/webhook', {
      method: 'POST',
  ...
  it('telegram webhook covers /link not found and /today empty', async () => {
      ...
      from: vi.fn((table: string) => {
        ...
        if (table === 'clients')
          return {
            ...
            update: vi.fn(() => ({
              eq: vi.fn(() => ({ eq: vi.fn(async () => ({ data: null, error: null })) })),
            })),
          } as any
        return { select: vi.fn(() => ({})) } as any
      }),
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      method: 'POST',
      body: JSON.stringify(body),
    } as any)
    res = await POST(req as any)
