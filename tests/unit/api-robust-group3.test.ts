...
describe('api robust group3', () => {
  ...
  describe('email/confirm', () => {
    ...
    it('404 cuando appointment no existe', async () => {
      ...
      const req = new NextRequest('http://localhost/api/email/confirm', {
        method: 'POST',
        headers: { authorization: 'Bearer secret123' },
        body: JSON.stringify({ appointmentId: 'uuid-1' }),
      } as any)
      const res = await POST(req as any)
    ...
    it('skipped: no client email', async () => {
      ...
      const req = new NextRequest('http://localhost/api/email/confirm', {
        ...
        body: JSON.stringify({ appointmentId: 'a1', formEmail: null }),
      } as any)
      const res = await POST(req as any)
  ...
  describe('user/locale', () => {
    it('400 invalid locale', async () => {
      ...
      const req = new NextRequest('http://localhost/api/user/locale', {
        method: 'POST',
  ...
  describe('clients/import', () => {
    it('429 rate limited', async () => {
      ...
      const req = new NextRequest('http://localhost/api/clients/import', {
        method: 'POST',
    ...
    it('422 validation failed max 500', async () => {
      ...
      const req = new NextRequest('http://localhost/api/clients/import', {
        method: 'POST',
        body: JSON.stringify({ clients: many }),
      } as any)
      const res = await POST(req as any)
    ...
    it('0 imported when all names empty', async () => {
      ...
      const req = new NextRequest('http://localhost/api/clients/import', {
        method: 'POST',
        body: JSON.stringify({ clients: [{ name: '' }, { name: '   ' }] }),
      } as any)
      const res = await POST(req as any)
      const j = await res.json()
    ...
    it('imports withPhone and withoutPhone', async () => {
      ...
      const mockUpsert = vi.fn(() => ({
        select: vi.fn(async () => ({ data: [{ id: 'c1' }, { id: 'c2' }], error: null })),
      }))
      const mockInsert = vi.fn(() => ({
        select: vi.fn(async () => ({ data: [{ id: 'c3' }], error: null })),
      }))
      vi.mocked(createClient).mockResolvedValue({
        auth: { getUser: async () => ({ data: { user: { id: 'u1' } }, error: null }) },
        from: vi.fn((table: string) => {
  ...
  describe('telegram webhook', () => {
    ...
    it('ok true sin message', async () => {
      const { POST } = await import('@/app/api/telegram/webhook/route')
      const req = new NextRequest('http://localhost/api/telegram/webhook?bid=biz1', {
        method: 'POST',
        body: JSON.stringify({}),
      } as any)
      const res = await POST(req as any)
    ...
    it('/start vincula owner', async () => {
      ...
      const { POST } = await import('@/app/api/telegram/webhook/route')
      const body = {
        message: { chat: { id: '123' }, text: '/start', from: { first_name: 'John' } },
      }
      const req = new NextRequest('http://localhost/api/telegram/webhook?bid=biz1', {
        method: 'POST',
