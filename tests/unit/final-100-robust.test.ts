...
vi.mock('serwist', () => ({
  Serwist: class { addEventListeners = vi.fn(); constructor(_opts:any){} },
  NetworkFirst: vi.fn(),
...
describe('final 100 robust', () => {
  ...
  it('register selfhosted without session triggers signIn and onboarding', async () => {
    ...
    vi.mocked(createClient).mockResolvedValue({
      auth:{
        signUp: vi.fn().mockResolvedValue({data:{user:{id:'u1', email:'a@b.com', user_metadata:{}}, session:null}, error:null}),
  ...
  it('appointments PATCH invalid json', async () => {
    const { createClient } = await import('@/lib/supabase/server')
    vi.mocked(createClient).mockResolvedValue({ auth:{ getUser: async()=>({data:{user:{id:'u1'}}, error:null}) }, from: vi.fn(()=>({ select: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:{id:'b1'}, error:null})) })) })) })) } as any)
    ...
    vi.mocked(createClient).mockResolvedValue({
      ...
      from: vi.fn((table:string)=>{
        if(table==='businesses') return {
          ...
          update: vi.fn(()=>({ eq: vi.fn(async()=>({error:{message:'slug exists'}})) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      })
    } as any)
    const { completeOnboarding } = await import('@/app/onboarding/actions')
    await expect(completeOnboarding({ bizType:'salon', bizName:'Biz', serviceName:'Cut', servicePrice:30, serviceDuration:30, slug:'valid-slug' })).rejects.toThrow('slug exists')
  })

  it('auth callback with next reset-password', async () => {
    const { createClient } = await import('@/lib/supabase/server')
    vi.mocked(createClient).mockResolvedValue({ auth:{ exchangeCodeForSession: vi.fn().mockResolvedValue({data:{user:{id:'u1', email:'a@b.com', user_metadata:{}}}, error:null}) } } as any)
    const { GET } = await import('@/app/auth/callback/route')
    const req = new NextRequest('http://localhost/auth/callback?code=abc&next=/reset-password' as any)
  ...
  it('auth callback existing onboarding not completed', async () => {
    ...
    const { createClient: createAdmin } = await import('@supabase/supabase-js')
    vi.mocked(createClient).mockResolvedValue({ auth:{ exchangeCodeForSession: vi.fn().mockResolvedValue({data:{user:{id:'u1', email:'a@b.com', user_metadata:{}}}, error:null}) } } as any)
    vi.mocked(createAdmin).mockReturnValue({ from: vi.fn(()=>({ select: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:{id:'b1', onboarding_completed:false}, error:null})) })) })) })) } as any)
    const { GET } = await import('@/app/auth/callback/route')
  ...
  it('email low-stock skip stock ok', async () => {
    ...
    ;(createServer as any).mockResolvedValue({ auth:{ getUser: async()=>({data:{user:{id:'u1'}}, error:null}) } } as any)
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((table:string)=>{
        if(table==='inventory_items') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'i1', name:'Item', quantity:10, unit:'pcs', low_stock_threshold:5, business_id:'b1'}, error:null})) })) })) } as any
      ...
      auth:{ admin:{ getUserById: vi.fn(async()=>({data:{user:{email:'a@b.com'}}})) } }
    } as any)
  ...
  it('viber set-webhook success', async () => {
    ...
    vi.mocked(createClient).mockResolvedValue({
      auth:{ getUser: async()=>({data:{user:{id:'u1'}}, error:null}) },
      from: vi.fn(()=>({ select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', viber_bot_token:'tok'}, error:null})) })) })) })),
    } as any)
  ...
  it('viber webhook conversation_started client', async () => {
    ...
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((table:string)=>{
        if(table==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:null}, error:null})) })) })) } as any
        if(table==='clients') return {
          select: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:{id:'client-uuid-123456789012345678901234', name:'Client'}, error:null})) })) })) })),
    ...
    const body = { event:'conversation_started', context:'client_123e4567-e89b-12d3-a456-426614174000', sender:{id:'user1', name:'John'} }
    const req = new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify(body)} as any)
    const res = await POST(req as any)
    expect(res.status).toBe(200)
  })
  ...
  it('viber webhook message /today', async () => {
    ...
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((table:string)=>{
        if(table==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', name:'Biz', viber_bot_token:'tok', viber_chat_id:'user1'}, error:null})) })) })) } as any
        if(table==='appointments') return {
          select: vi.fn(()=>({
            eq: vi.fn(()=>({
              gte: vi.fn(()=>({ lte: vi.fn(()=>({ order: vi.fn(async()=>({data:[{starts_at:new Date().toISOString(), status:'confirmed', clients:{name:'Client'}, services:{name:'Cut'} }], error:null})) })) })),
            })),
          })),
        } as any
        return { select: vi.fn(()=>({})) } as any
  ...
  it('cron email confirm full with dedup', async () => {
    ...
    const appt = { id:'a1', starts_at:'2026-01-15T10:00:00Z', business_id:'b1', source:'online', services:{name:'Cut', duration_min:30}, employees:{name:'John'}, clients:{name:'Client', email:'c@test.com', whatsapp_number:null, telegram_id:null, viber_user_id:null} }
    const biz = { name:'Biz', address:'Addr', slug:'biz', timezone:'UTC', telegram_bot_token:null, telegram_chat_id:null, viber_bot_token:null, viber_chat_id:null, meta_whatsapp_phone_number_id:null, meta_whatsapp_access_token:null, email:'biz@test.com' }
    ...
    const req = new NextRequest('http://localhost/api/email/confirm', { method:'POST', headers:{ authorization:'Bearer s3cret' }, body: JSON.stringify({appointmentId:'a1', formEmail:'form@test.com'}) } as any)
    const res = await POST(req as any)
    const j = await res.json()
