...
describe('telegram lowstock 100', () => {
  ...
  it('telegram webhook covers /start client not found and /link', async () => {
    ...
    let body = {
      message: {
        chat: { id: '1' },
        text: '/start client_123e4567-e89b-12d3-a456-426614174000',
    ...
    ;(supaMod.createClient as any).mockReturnValue({
      from: vi.fn((table: string) => {
        if (table === 'inventory_items')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: item, error: null })) })),
            })),
          } as any
        if (table === 'businesses') {
          const c: any = {}
  ...
  it('telegram set-webhook invalid token and webhook fail', async () => {
    ...
    vi.mocked(getTelegramBotInfo).mockResolvedValueOnce({
      ok: true,
      result: { username: 'bot' },
    } as any)
