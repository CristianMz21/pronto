...
describe('US1 — book integration: Zod, DomPurify, rateLimit, slot_taken vs no_staff_available, Anyone', () => {
  ...
  it('slot_taken vs no_staff_available distinguished (034)', async () => {
    ...
    const serviceChain = makeChain({
      data: { id: 'bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbbb', duration_min: 30, price: 30000 },
      error: null,
    })
    const bizChain = makeChain({
      data: { timezone: 'UTC', min_advance_minutes: 30, booking_lead_time_enabled: true },
      error: null,
    })
    ...
    const req = new NextRequest('http://localhost/api/book', {
      method: 'POST',
      headers: { 'x-forwarded-for': '1.1.1.1' } as any,
      body: JSON.stringify(validPayload({ date: '2099-06-15', time: '10:00' })),
    })
    const res = await BookPOST(req as any)
    ...
    const req2 = new NextRequest('http://localhost/api/book', {
      method: 'POST',
      headers: { 'x-forwarded-for': '1.1.1.1' } as any,
      body: JSON.stringify(validPayload()),
    })
  ...
  it('Anyone (employeeId null) auto-assign allowed and passes validation', async () => {
    ...
    const req = new NextRequest('http://localhost/api/book', {
      method: 'POST',
      headers: { 'x-forwarded-for': '1.1.1.1' } as any,
      body: JSON.stringify(validPayload({ employeeId: null })),
    })
    const res = await BookPOST(req as any)
  ...
  it('DomPurify sanitizes name (script stripped)', async () => {
    ...
    const req = new NextRequest('http://localhost/api/book', {
      method: 'POST',
      headers: { 'x-forwarded-for': '1.1.1.1' } as any,
      body: JSON.stringify(validPayload({ name: '<script>alert(1)</script>Ana' })),
    })
    const res = await BookPOST(req as any)
    expect(res.status).toBe(200)
