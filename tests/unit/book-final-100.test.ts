...
describe('book final 100 strict', () => {
  ...
  it('covers claim user_id !== null with insert success and fallback failure (191-220)', async () => {
    ...
    function makeChain(data: any) {
      const c: any = {}
      ;['select', 'eq', 'or', 'limit', 'maybeSingle', 'single', 'gte', 'lte', 'not'].forEach(
        (m) => (c[m] = vi.fn(() => c)),
      )
        ...
        if (t === 'clients') {
          ...
          if (clientCall === 3)
            return {
              ...
              update: vi.fn(() => ({})),
            } as any // insert will be called via from('clients').insert
          // fallback fetch
          if (clientCall === 4)
            return makeChain({ id: 'c_fallback', telegram_id: 'tg2', viber_user_id: 'vb2' })
  ...
  it('covers 223-240 no match create new client success vs failure', async () => {
    ...
    function makeChain(data: any) {
      const c: any = {}
      ;['select', 'eq', 'or', 'limit', 'maybeSingle', 'single'].forEach(
        (m) => (c[m] = vi.fn(() => c)),
      )
      c.maybeSingle = vi.fn(async () => ({
        data: Array.isArray(data) ? (data[0] ?? null) : data,
      ...
      from: vi.fn((t: string) => {
        ...
        if (t === 'appointments')
          return {
            insert: vi.fn(() => ({
              select: vi.fn(() => ({
                single: vi.fn(async () => ({ data: { id: 'a_new' }, error: null })),
              })),
            })),
          } as any
        return makeChain(null)
