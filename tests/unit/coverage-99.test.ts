...
vi.mock('isomorphic-dompurify', () => ({ default:{ sanitize: (s:string)=> String(s).replace(/<[^>]*>/g,'').trim().slice(0,1000) }}))
...
describe('coverage 99 strict', ()=>{
  ...
  it('clients/import covers upsert error 102 and insert error 127', async()=>{
    ...
    vi.mocked(createClient).mockResolvedValue({
      ...
      from: vi.fn((t:string)=>{
        ...
        if(t==='clients') return {
          upsert: vi.fn(()=>({ select: vi.fn(async()=>({data:null, error:{message:'upsert fail'}})) })),
          insert: vi.fn(()=>({ select: vi.fn(async()=>({data:[{id:'c1'}], error:null})) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    let req=new NextRequest('http://localhost/api/clients/import', { method:'POST', body: JSON.stringify({clients:[{name:'A', phone:'+1'}]})} as any)
    ...
    vi.mocked(createClient).mockResolvedValue({
      ...
      from: vi.fn((t:string)=>{
        ...
        if(t==='clients') return {
          ...
          insert: vi.fn(()=>({ select: vi.fn(async()=>({data:null, error:{message:'insert fail'}})) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    req=new NextRequest('http://localhost/api/clients/import', { method:'POST', body: JSON.stringify({clients:[{name:'B'}]})} as any)
  ...
  it('cron covers 154 and 226 error logs', async()=>{
    ...
    vi.spyOn(emailMod as any, 'sendReminder').mockImplementation(async()=> { throw new Error('fail') })
    process.env.CRON_SECRET='secret'
    const svc=await import('@supabase/supabase-js')
    const biz={ name:'Biz', timezone:'UTC', telegram_bot_token:null, viber_bot_token:null, meta_whatsapp_phone_number_id:null } as any
    const client={ name:'Cli', email:'c@test.com', whatsapp_number:null, viber_user_id:null, telegram_id:null }
    const appt24={ id:'a1', business_id:'b1', starts_at:new Date(Date.now()+24*3600*1000).toISOString(), services:{name:'S'}, employees:{name:'E'}, clients:client }
    const appt1h={ id:'a2', business_id:'b1', starts_at:new Date(Date.now()+60*60*1000).toISOString(), services:{name:'S'}, employees:{name:'E'}, clients:client }
    let call=0
    vi.mocked(svc.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='appointments'){
          ...
          if(call===1) c.then=(r:any)=> Promise.resolve({data:[appt24], error:null}).then(r)
          else if(call===2) c.then=(r:any)=> Promise.resolve({data:[appt1h], error:null}).then(r)
        ...
        if(t==='clients'){
          const c:any={}; ['select','gte','lte','not'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[], error:null}).then(r); return c
        }
  ...
  it('email confirm covers 239 log error and viber set-webhook 37,46,57', async()=>{
    ...
    const appt={ id:'a1', starts_at:'2026-01-15T10:00:00Z', business_id:'b1', services:{name:'Cut', duration_min:30}, employees:{name:'Bob'}, clients:{name:'Alice', email:'alice@test.com', whatsapp_number:null, telegram_id:null, viber_user_id:null}}
    const biz={ name:'Biz', address:null, timezone:'UTC', telegram_bot_token:null, viber_bot_token:null, meta_whatsapp_phone_number_id:null } as any
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        ...
        if(t==='notification_log') return {
          ...
          insert: vi.fn(async()=>({error:{code:'23506', message:'insert fail', code:'23506'}})),
        } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    ...
    let vbReq=new NextRequest('http://localhost/api/viber/set-webhook', { method:'POST', body: JSON.stringify({}) } as any)
    ...
    vi.spyOn(viberMod as any, 'setViberWebhook').mockResolvedValue({ok:false, description:'fail set'} as any)
    vbReq=new NextRequest('http://localhost/api/viber/set-webhook', { method:'POST', body: JSON.stringify({}) } as any)
  ...
  it('covers sitemap 76, proxy 49, auth callback 53-54,79, onboarding 78', async()=>{
    ...
    vi.mocked(srv.createClient).mockResolvedValue({
      auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1'}}, error:null})) },
      from: vi.fn((t:string)=>{
        if(t==='businesses') return { select: vi.fn(async()=>({data:[{slug:null, updated_at:'2026-01-01'}], error:null})) } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    ...
    vi.mocked(createServerClient).mockImplementation((_u:any,_k:any,opts:any)=>{
      return {
        auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1', email:'a@b.com'}}, error:null})) },
        // trigger setAll
        getAll: opts.cookies.getAll,
        setAll: (cookies:any[])=>{ setAllCalled=true; opts.cookies.setAll(cookies) },
      } as any
    })
    ...
    const req:any={ nextUrl: url, cookies:{ get: vi.fn(()=>null), getAll: vi.fn(()=>[]), set: vi.fn() }, headers: new Headers() }
    ...
    vi.mocked(srv.createClient).mockResolvedValue({
      auth:{
        exchangeCodeForSession: vi.fn(async()=>({data:{session:{access_token:'tok'}}, error:null})),
    ...
    vi.mocked(srv.createClient).mockResolvedValue({
      ...
      from: vi.fn((t:string)=>{
        if(t==='businesses') return {
          select: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:{id:'b1', slug:'biz'}, error:null})) })) })),
          update: vi.fn(()=>({ eq: vi.fn(async()=>({error:{message:'slug taken'}})) })),
        } as any
    ...
    await expect(completeOnboarding({bizType:'salon', bizName:'Test', serviceName:'Cut', servicePrice:10, serviceDuration:30, slug:'valid-slug'})).rejects.toThrow('slug taken')
  })
  ...
  it('covers viber webhook remaining 77,94,126-131,182', async()=>{
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        ...
        if(t==='clients') return {
          select: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:{id:'c1'}, error:null})) })) })) })),
          update: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(async()=>({error:null})) })) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'conversation_started', user:{id:'u1'}, subscribed:true})} as any)
    res=await POST(req as any)
    expect(res.status).toBe(200)
