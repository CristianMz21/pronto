...
import { computeEffectiveHours, checkSlotWithinHours, dayOfWeekFromDateString, DEFAULT_HOURS } from '@/lib/booking-availability'
import fc from 'fast-check'

describe('booking-availability strict 100%', () => {
  describe('DEFAULT_HOURS', () => {
    it('7 days', () => { expect(DEFAULT_HOURS.length).toBe(7) })
  ...
  describe('computeEffectiveHours', () => {
    ...
    it('fills missing days', () => {
      const custom = [{ day_of_week: 1, is_open: false, open_time: '10:00', close_time: '18:00', break_start: null, break_end: null }]
    ...
    it('keeps all provided', () => {
      const all = [0,1,2,3,4,5,6].map(d => ({ day_of_week: d, is_open: true, open_time: '08:00', close_time: '22:00', break_start: null, break_end: null }))
      const h = computeEffectiveHours(all as any)
      expect(h.every(x => x.open_time === '08:00')).toBe(true)
    })
    it('preserves provided break', () => {
      const custom = [{ day_of_week: 3, is_open: true, open_time: '09:00', close_time: '20:00', break_start: '12:00', break_end: '13:00' }]
      const h = computeEffectiveHours(custom as any)
      expect(h[3].break_start).toBe('12:00')
    })
  })
  describe('checkSlotWithinHours', () => {
    it('undefined -> closed', () => { expect(checkSlotWithinHours(undefined, '10:00', 30)).toEqual({ ok: false, reason: 'closed' }) })
    it('is_open false -> closed', () => { expect(checkSlotWithinHours({ day_of_week:1,is_open:false,open_time:'09:00',close_time:'20:00' }, '10:00', 30).ok).toBe(false) })
    it('slot exactly at open', () => {
      const dh = { day_of_week:1,is_open:true,open_time:'09:00',close_time:'20:00',break_start:null,break_end:null }
    ...
    it('break overlap -> break', () => {
      const dh = { day_of_week:1,is_open:true,open_time:'09:00',close_time:'20:00',break_start:'12:00',break_end:'13:00' }
    ...
    it('only break_start without break_end -> no break check', () => {
      const dh = { day_of_week:1,is_open:true,open_time:'09:00',close_time:'20:00',break_start:'12:00',break_end:null } as any
      expect(checkSlotWithinHours(dh,'12:30',30)).toEqual({ ok: true })
    })
    it('only break_end without break_start -> no break', () => {
      const dh = { day_of_week:1,is_open:true,open_time:'09:00',close_time:'20:00',break_start:null,break_end:'13:00' } as any
      expect(checkSlotWithinHours(dh,'12:30',30)).toEqual({ ok: true })
    })
    ...
    it('property: result is always {ok:true} or {ok:false,reason}', () => {
      fc.assert(fc.property(fc.integer({min:0,max:6}), fc.string(), fc.integer({min:0,max:1440}), (dow, time, dur) => {
        const dh = { day_of_week: dow, is_open: true, open_time: '09:00', close_time: '20:00', break_start: null, break_end: null }
  ...
  describe('dayOfWeekFromDateString', () => {
    it('known dates', () => {
      ...
      expect(dayOfWeekFromDateString('2026-01-15')).toBe(new Date(Date.UTC(2026,0,15)).getUTCDay())
    })
