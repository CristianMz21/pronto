...
describe('massive final coverage', () => {
  ...
  it('email/confirm covers all channels (telegram owner+client, viber owner+client, whatsapp)', async () => {
    ...
    const req = new NextRequest('http://localhost/api/email/confirm', {
      ...
      body: JSON.stringify({ appointmentId: 'a1', formEmail: 'alice@test.com' }),
    } as any)
  ...
  it('email/confirm covers dedup alreadySent and no email skip', async () => {
            ...
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  eq: vi.fn(() => ({
                    eq: vi.fn(() => ({
                      maybeSingle: vi.fn(async () => ({ data: { id: 'log1' }, error: null })),
                    })),
                  })),
                })),
              })),
    ...
    let req = new NextRequest('http://localhost/api/email/confirm', {
      method: 'POST',
      headers: { authorization: 'Bearer s3cret' },
      body: JSON.stringify({ appointmentId: 'a2' }),
    } as any)
    ...
    ;(supa.createClient as any).mockReturnValue({
      from: vi.fn((t: string) => {
        if (t === 'appointments')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: appt2, error: null })) })),
            })),
          } as any
        if (t === 'businesses')
          return {
    ...
    req = new NextRequest('http://localhost/api/email/confirm', {
      method: 'POST',
      headers: { authorization: 'Bearer s3cret' },
      body: JSON.stringify({ appointmentId: 'a2' }),
    } as any)
  ...
  it('email/confirm covers log insert error 23505 ignored and other error logged', async () => {
    ...
    const appt = {
      ...
      starts_at: '2026-01-15T10:00:00Z',
      business_id: 'b1',
      services: { name: 'Cut', duration_min: 60 },
      employees: null,
      clients: {
        name: 'Dave',
        email: 'd@test.com',
    ...
    const req = new NextRequest('http://localhost/api/email/confirm', {
      method: 'POST',
      headers: { authorization: 'Bearer s3cret' },
      body: JSON.stringify({ appointmentId: 'a3' }),
    } as any)
    const res = await POST(req as any)
    expect(res.status).toBe(200)
  })
  ...
  it('telegram webhook covers client_ with phone, email, no contact, not found, invalid uuid', async () => {
              ...
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  maybeSingle: vi.fn(async () => ({
                    data: { id: 'c1', name: 'john doe', phone: '+123', email: null },
                    error: null,
                  })),
                })),
              })),
    ...
    let req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      ...
      body: JSON.stringify({
        message: {
          chat: { id: '1' },
          text: '/start client_11111111-1111-1111-1111-111111111111',
          ...
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  maybeSingle: vi.fn(async () => ({
                    data: { id: 'c2', name: 'jane', phone: null, email: 'j@e.com' },
                    error: null,
                  })),
                })),
              })),
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      ...
      body: JSON.stringify({
        message: {
          chat: { id: '2' },
          text: '/start client_22222222-2222-2222-2222-222222222222',
          ...
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  maybeSingle: vi.fn(async () => ({
                    data: { id: 'c3', name: 'bob', phone: null, email: null },
                    error: null,
                  })),
                })),
              })),
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      ...
      body: JSON.stringify({
        message: {
          chat: { id: '3' },
          text: '/start client_33333333-3333-3333-3333-333333333333',
          from: { first_name: 'Bob' },
        },
      }),
    } as any)
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      ...
      body: JSON.stringify({
        message: {
          chat: { id: '4' },
          text: '/start client_44444444-4444-4444-4444-444444444444',
          from: { first_name: 'Nope' },
        },
      }),
    } as any)
    res = await POST(req as any)
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      method: 'POST',
      body: JSON.stringify({
        message: { chat: { id: '5' }, text: '/start client_invalid', from: { first_name: 'Bad' } },
      }),
    } as any)
    res = await POST(req as any)
    expect(res.status).toBe(200)
  ...
  it('telegram webhook covers /link with found and not found and /today with data', async () => {
    ...
    let req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      method: 'POST',
      body: JSON.stringify({
        message: { chat: { id: '1' }, text: '/link +123', from: { first_name: 'John' } },
      }),
    } as any)
    let res = await POST(req as any)
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      method: 'POST',
      body: JSON.stringify({
        message: { chat: { id: '1' }, text: '/link +999', from: { first_name: 'John' } },
      }),
    } as any)
    res = await POST(req as any)
    expect(res.status).toBe(200)
    ...
    req = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
      method: 'POST',
      body: JSON.stringify({
        message: { chat: { id: '1' }, text: '/today', from: { first_name: 'Owner' } },
      }),
    } as any)
    res = await POST(req as any)
    expect(res.status).toBe(200)
  ...
  it('covers cron birthday filter and notification channels', async () => {
    ...
    const bizFull = {
      ...
      viber_bot_token: 'vbt',
      viber_chat_id: 'vc',
      meta_whatsapp_phone_number_id: 'pn',
      meta_whatsapp_access_token: 'at',
      timezone: 'UTC',
    }
    const clientBday = {
      ...
      viber_user_id: 'v1',
      telegram_id: 'tg1',
      birthday: `1990-${todayMD}`,
      business_id: 'b1',
    }
    const clientNoChannel = {
      id: 'c2',
      name: 'NoCh',
      email: null,
      whatsapp_number: null,
    ...
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((t: string) => {
        if (t === 'appointments') {
          ...
          const c: any = {}
          ;['select', 'eq', 'gte', 'lte', 'not'].forEach((m) => (c[m] = vi.fn(() => c)))
            ...
            if (isNot)
              return Promise.resolve({
                data: [
                  clientBday,
                  clientNoChannel,
                  { id: 'c3', birthday: null, business_id: 'b1' },
                ],
                error: null,
              }).then(r)
        ...
        if (t === 'businesses')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: bizFull, error: null })) })),
            })),
          } as any
    ...
    vi.mocked(createServiceClient).mockReturnValue({
      from: vi.fn((t: string) => {
        ...
        if (t === 'business_hours')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(async () => ({ data: businessHoursClosed, error: null })),
            })),
          } as any
        if (t === 'clients')
          return {
