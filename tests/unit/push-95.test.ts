...
describe('push 95', () => {
  ...
  it('email confirm covers no biz and no client', async () => {
    ...
    const req = new NextRequest('http://localhost/api/email/confirm', {
      ...
      body: JSON.stringify({ appointmentId: 'a1', formEmail: 'test@test.com' }),
    } as any)
      ...
      from: vi.fn((table: string) => {
        if (table === 'appointments')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(async () => ({ data: null, error: { message: 'fetch fail' } })),
              })),
            })),
          } as any
        if (table === 'businesses')
  ...
  it('cron covers thankyou with no biz slug and reactivation with no biz', async () => {
    ...
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((table: string) => {
        ...
        if (table === 'clients') {
          const c: any = {}
          ;['select', 'gte', 'lte', 'not', 'eq'].forEach((m) => (c[m] = vi.fn(() => c)))
        ...
        if (table === 'businesses') {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ single: vi.fn(async () => ({ data: bizNoSlug, error: null })) })),
            })),
          } as any
        }
        if (table === 'notification_log') {
  ...
  it('telegram webhook covers all branches', async () => {
    ...
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
            single: vi.fn(async () => ({
              data: { id: 'b1', telegram_bot_token: null },
              error: null,
            })),
          })),
        })),
    ...
    for (const c of cases) {
      ...
      const r = new NextRequest('http://localhost/api/telegram/webhook?bid=b1', {
        method: 'POST',
