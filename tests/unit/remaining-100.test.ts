...
describe('remaining 100 strict', () => {
      ...
      from: vi.fn((t: string) => {
        ...
        if (t === 'clients') {
          ...
          return {
            ...
            insert: vi.fn(() => ({
              select: vi.fn(() => ({
                single: vi.fn(async () => ({ data: { id: 'c_new' }, error: null })),
              })),
            })),
          } as any
        }
        ...
        if (t === 'clients') {
          ...
          if (call === 3)
            return {
              insert: vi.fn(() => ({
                select: vi.fn(() => ({
                  single: vi.fn(async () => ({ data: { id: 'c_created' }, error: null })),
                })),
              })),
  ...
  it('covers clients/import 102-103 and 127-128 and cron 154,226 and email confirm 60,239', async () => {
      ...
      from: vi.fn((t: string) => {
        ...
        if (t === 'clients')
          return {
            ...
            insert: vi.fn(() => ({
              select: vi.fn(async () => ({ data: [{ id: 'c2' }], error: null })),
            })),
          } as any
    ...
    let req = new NextRequest('http://localhost/api/clients/import', {
      method: 'POST',
      body: JSON.stringify({ clients: [{ name: 'A', phone: '+1' }, { name: 'B' }] }),
    } as any)
        ...
        if (t === 'businesses')
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(async () => ({
                  data: { id: 'b1', viber_bot_token: 'tok', viber_chat_id: 'tc' },
                  error: null,
                })),
              })),
            })),
  ...
  it('covers client appointments 107,110,126 strictly', async () => {
    ...
    let req = new NextRequest('http://localhost/api/client/appointments/a1', {
      method: 'PUT',
      body: JSON.stringify({ date: futureDate, time: futureTime }),
    } as any)
    ...
    req = new NextRequest('http://localhost/api/client/appointments/a1', {
      method: 'PUT',
      body: JSON.stringify({ date: validDate, time: '10:00' }),
    } as any)
