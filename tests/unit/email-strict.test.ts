...
describe('email strict 100%', () => {
  ...
  it('sendBookingConfirmation with employee, address, calendarUrl XSS', async () => {
    await sendBookingConfirmation({
      ...
      date: 'd',
      time: 't',
      employeeName: 'Bob<script>',
      address: 'Addr <b>',
      calendarUrl: 'https://cal.com?x="onload="alert(1)',
    })
  ...
  it('sendThankYou with and without bookingUrl', async () => {
    ...
    await sendThankYou({
      ...
      businessName: 'B',
      serviceName: 'Cut',
      bookingUrl: 'https://book.com/?x=`code`',
    })
  ...
  it('sendLowStockAlert single and multiple', async () => {
    await sendLowStockAlert({
      to: 'a@test.com',
      businessName: 'B',
      items: [{ name: 'Item <b>', quantity: 1, unit: 'pcs', threshold: 5 }],
    })
    let html = vi.mocked(sendMail).mock.calls[0][0].html
    ...
    await sendLowStockAlert({
      ...
      businessName: 'B',
      items: [
        { name: 'A', quantity: 1, unit: 'u', threshold: 5 },
        { name: 'B', quantity: 2, unit: 'u', threshold: 5 },
      ],
    })
    html = vi.mocked(sendMail).mock.calls[0][0].html
