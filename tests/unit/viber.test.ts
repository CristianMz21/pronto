...
describe('viber exhaustive', () => {
  it('send', async () => {
    ...
    ;(global.fetch as any).mockResolvedValue({ json: async () => ({ status: 1 }) } as any)
    expect(await sendViberMessage('tok', 'user', 'hi')).toBe(false)
  })
  it('tpl', () => {
    expect(
      tplNewBooking({ clientName: 'A', serviceName: 'S', date: 'd', time: 't', source: 'online' }),
    ).toContain('(online)')
    expect(
