...
describe('create-business strict 100%', () => {
  ...
  it('inserts with full_name', async () => {
    ...
    await insertOwnerAsEmployee(admin, 'biz1', { email: 'test@example.com', user_metadata: { full_name: '  John Doe  ' } })
  ...
  it('derive from email local part with separators', async () => {
    const admin = makeAdmin()
    await insertOwnerAsEmployee(admin, 'biz', { email: 'john.doe_smith-test@example.com', user_metadata: {} })
    const arg = admin.from.mock.results[0].value.insert.mock.calls[0][0]
  ...
  it('empty full_name string falls to email', async () => {
    const admin = makeAdmin()
    await insertOwnerAsEmployee(admin, 'biz', { email: 'a@b.com', user_metadata: { full_name: '   ' } })
    const arg = admin.from.mock.results[0].value.insert.mock.calls[0][0]
    expect(arg.name).toBe('A')
  })
  it('full_name not string falls to email', async () => {
    const admin = makeAdmin()
    await insertOwnerAsEmployee(admin, 'biz', { email: 'a@b.com', user_metadata: { full_name: 123 as any } })
    const arg = admin.from.mock.results[0].value.insert.mock.calls[0][0]
    expect(arg.name).toBe('A')
  })
  ...
  it('handles error logs console.error without throwing', async () => {
    ...
    await expect(insertOwnerAsEmployee(admin, 'biz', { email: 'a@b.com', user_metadata: null })).resolves.toBeUndefined()
    expect(spy).toHaveBeenCalledWith(expect.stringContaining('[create-business]'), expect.stringContaining('duplicate'))
    spy.mockRestore()
  })
  it('email with dots and dashes normalized TitleCase', async () => {
    const admin = makeAdmin()
    await insertOwnerAsEmployee(admin, 'biz', { email: 'maría_josé.perez-lopez@example.com', user_metadata: {} })
    const arg = admin.from.mock.results[0].value.insert.mock.calls[0][0]
