...
describe('final viber 100 strict', ()=>{
  ...
  it('viber webhook covers 77,94,126-131,182', async()=>{
    ...
    req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'conversation_started', user:{id:'u1', name:'John'}, subscribed:true})} as any)
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', viber_bot_token:'tok'}, error:null})) })) })) } as any
        if(t==='appointments') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ gte: vi.fn(()=>({ lte: vi.fn(()=>({ order: vi.fn(async()=>({data:[{starts_at:new Date().toISOString(), status:'confirmed', clients:{name:'Alice'}, services:{name:'Cut'}}, {starts_at:new Date().toISOString(), status:'cancelled', clients:{name:'Bob'}, services:{name:'Color'}}], error:null})) })) })) })) })) } as any
        return { select: vi.fn(()=>({})) } as any
      }),
    } as any)
    ...
    req=new NextRequest('http://localhost/api/viber/webhook', { method:'POST', body: JSON.stringify({ event:'message'})} as any)
    res=await POST(req as any)
    ...
    res=await POST(req as any)
    expect(res.status).toBe(200)
    req=new NextRequest('http://localhost/api/viber/webhook?bid=b1', { method:'POST', body: JSON.stringify({ event:'message', sender:{id:'u1'}, message:{text:'/link +123'}})} as any)
    res=await POST(req as any)
    expect(res.status).toBe(200)
  })
  ...
  it('covers viber and telegram set-webhook remaining 37,46,57 and proxy 49', async()=>{
    ...
    let req=new NextRequest('http://localhost/api/viber/set-webhook', { method:'POST', body: JSON.stringify({}) } as any)
    let res=await vbPost(req as any)
    expect(res.status).toBe(200)
    expect((await res.json()).ok).toBe(true)

    // 37: setViberWebhook fail
    vi.spyOn(viberMod as any, 'setViberWebhook').mockResolvedValue({ok:false, description:'fail'} as any)
    ...
    vi.spyOn(tgMod as any, 'getTelegramBotInfo').mockResolvedValue({ok:true, result:{username:'bot'}} as any)
    ...
    expect(res.status).toBe(400)
    vi.spyOn(tgMod as any, 'getTelegramBotInfo').mockResolvedValue({ok:true, result:{username:'bot'}} as any)
    vi.spyOn(tgMod as any, 'setTelegramWebhook').mockResolvedValue({ok:false, description:'fail'} as any)
    req=new NextRequest('http://localhost/api/telegram/set-webhook', { method:'POST', body: JSON.stringify({}) } as any)
    res=await tgPost(req as any)
    expect(res.status).toBe(400)
  ...
  it('covers onboarding 78, auth callback 53-54,79, sitemap 76, clients/import 41', async()=>{
    ...
    await expect(completeOnboarding({bizType:'salon', bizName:'Test', serviceName:'Cut', servicePrice:10, serviceDuration:30, slug:'valid-slug'})).rejects.toThrow('NEXT_REDIRECT:https://valid-slug.trypronto.app/dashboard')
    ...
    vi.mocked(srv.createClient).mockResolvedValue({
      ...
      from: vi.fn(()=>({ select: vi.fn(async()=>({data:[{slug:null, updated_at:'2026-01-01'}, {slug:'biz', updated_at:'2026-01-01'}], error:null})) })),
    } as any)
    ...
    req=new NextRequest('http://localhost/api/clients/import', { method:'POST', body: JSON.stringify({clients:[]})} as any)
