...
describe('sprint 99 - final gaps', ()=>{
  ...
  it('register covers businessName missing, signUpError, selfhosted, no session, collision', async()=>{
    ...
    vi.mocked(createClient).mockResolvedValue({ auth:{ signUp: vi.fn(), signInWithPassword: vi.fn() } } as any)
    let fd = new FormData(); fd.set('email','a@b.com'); fd.set('password','12345678'); fd.set('business_name','')
    await expect(register(fd)).rejects.toThrow('NEXT_REDIRECT:/register?error=Business')

    // signUpError
    vi.mocked(createClient).mockResolvedValue({
      auth:{ signUp: vi.fn(async()=>({data:{}, error:{message:'email taken'}})), signInWithPassword: vi.fn() }
    } as any)
    fd = new FormData(); fd.set('email','a@b.com'); fd.set('password','12345678'); fd.set('business_name','Biz')
    await expect(register(fd)).rejects.toThrow('NEXT_REDIRECT:/register?error=email%20taken')

    // success with session -> onboarding
    vi.mocked(createClient).mockResolvedValue({
      auth:{ signUp: vi.fn(async()=>({data:{user:{id:'u1', email:'a@b.com', user_metadata:{full_name:'Test'}}, session:{access_token:'tok'}}, error:null})), signInWithPassword: vi.fn() }
    } as any)
    ...
    vi.mocked(createClient).mockResolvedValue({
      auth:{
        signUp: vi.fn(async()=>({data:{user:{id:'u1', email:'a@b.com', user_metadata:{full_name:'Test'}}, session:null}, error:null})),
  ...
  it('loginWithGoogle covers error and success', async()=>{
    ...
    let fd = new FormData(); fd.set('redirectTo','/dashboard')
    ...
    // success
    vi.mocked(createClient).mockResolvedValue({
      auth:{ signInWithOAuth: vi.fn(async()=>({data:{url:'https://google.com/auth'}, error:null})) }
    } as any)
  ...
  it('client register covers session fallback branches 33-37', async()=>{
    ...
    let fd = new FormData(); fd.set('email','a@b.com'); fd.set('password','pass123'); fd.set('name','John'); fd.set('phone','+1')
  ...
  it('book covers barber triggers and client creation errors', async()=>{
    ...
    const triggers = ['no_staff_available','slot_already_booked','barber_not_qualified','barber_unavailable','barber_inactive','outside_availability closed','in_past']
      ...
      vi.mocked(createServiceClient).mockReturnValue({
        from: vi.fn((t:string)=>{
          ...
          if(t==='appointments') return {
            insert: vi.fn(()=>({ select: vi.fn(()=>({ single: vi.fn(async()=>({data:null, error:{message:msg}})) })) })),
          } as any
    ...
    vi.mocked(createServiceClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        ...
        if(t==='clients') return {
          select: vi.fn(()=>({ eq: vi.fn(()=>({ or: vi.fn(()=>({ limit: vi.fn(async()=>({data:[], error:null})) })) })) })),
          insert: vi.fn(()=>({ select: vi.fn(()=>({ single: vi.fn(async()=>({data:null, error:{message:'dup'}})) })) })),
          update: vi.fn(()=>({ eq: vi.fn(async()=>({error:null})) })),
        } as any
        return { select: vi.fn(()=>({})) } as any
      })
    ...
    const { POST } = await import('@/app/api/book/route')
    let req = new NextRequest('http://localhost/api/book', { method:'POST', body: JSON.stringify({ businessId:'11111111-1111-1111-1111-111111111111', serviceId:'22222222-2222-2222-2222-222222222222', date: futureDate, time:'10:00', name:'John', phone:'+123'})} as any)
  ...
  it('telegram and viber set-webhook cover all branches', async()=>{
    ...
    vi.mocked(createClient).mockResolvedValue({
      ...
      from: vi.fn(()=>({ select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', telegram_bot_token:null, viber_bot_token:null}, error:null})) })) })) })),
    } as any)
    req = new NextRequest('http://localhost/api/telegram/set-webhook', { method:'POST', body: JSON.stringify({}) } as any)
    ...
    vi.mocked(createClient).mockResolvedValue({
      auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1'}}, error:null})) },
      from: vi.fn(()=>({ select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'b1', telegram_bot_token:'tok', viber_bot_token:'vbt'}, error:null})) })) })) })),
    } as any)
    ...
    vi.mocked(tmod.setTelegramWebhook).mockResolvedValue({ok:false, description:'hook fail'} as any)
    vi.mocked(vmod.setViberWebhook).mockResolvedValue({ok:false, description:'hook fail'} as any)
    // to reach setWebhook, we need APP_URL https and token valid, so set get to ok
    vi.mocked(tmod.getTelegramBotInfo).mockResolvedValue({ok:true, result:{username:'bot'}} as any)
  ...
  it('covers proxy setAll and config matcher', async()=>{
    ...
    vi.mocked(createServerClient).mockImplementation((_url:any,_key:any, opts:any)=>{
      ...
      return { auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1', email:'a@b.com'}}, error:null})) } } as any
    })
    ...
    const req:any = {
      nextUrl: url,
      cookies: { get: vi.fn(()=>null), getAll: vi.fn(()=> [{name:'a', value:'1'}]), set: vi.fn() },
      headers: new Headers(),
    }
  ...
  it('covers clients/import auth and validation', async()=>{
    ...
    const { createClient } = await import('@/lib/supabase/server')
    vi.mocked(createClient).mockResolvedValue({ auth:{ getUser: vi.fn(async()=>({data:{user:null}, error:null})) } } as any)
    let req = new NextRequest('http://localhost/api/clients/import', { method:'POST', body: JSON.stringify({clients:[]})} as any)
    let res = await POST(req as any)
    ...
    req = new NextRequest('http://localhost/api/clients/import', { method:'POST', body: JSON.stringify({clients:[{name:'A'}]})} as any)
