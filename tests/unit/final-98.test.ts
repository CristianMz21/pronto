...
vi.mock('@/lib/email', () => ({ sendLowStockAlert: vi.fn().mockResolvedValue({}), sendBookingConfirmation: vi.fn().mockResolvedValue({}) }))
...
vi.mock('@supabase/ssr', () => ({
  createServerClient: vi.fn(()=> ({ auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1', email:'u@test.com'}}, error:null})) } }))
}))
...
describe('final 98 push', ()=>{
  ...
  it('low-stock covers all branches', async()=>{
    ...
    function mockService(item:any, ownership:any, already:any, biz:any, authUser:any){
      const mock:any = {
        ...
        from: vi.fn((t:string)=>{
          if(t==='inventory_items') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data:item, error: item ? null : {message:'not found'}})) })) })) } as any
          if(t==='businesses'){
          ...
          if(t==='notification_log') return {
            select: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data: already, error:null})) })) })) })) })) })),
    ...
    // 1. unauthorized
    vi.mocked(srv.createClient).mockResolvedValue({ auth:{ getUser: vi.fn(async()=>({data:{user:null}, error:null})) } } as any)
    ...
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({}) } as any)
    ...
    // 4. forbidden
    const item = { id:'i1', name:'Item', quantity:2, unit:'pcs', low_stock_threshold:10, business_id:'b1' }
    ...
    vi.mocked(supa.createClient).mockReturnValue(mockService(itemOk, {id:'b1'}, null, null, null) as any)
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({itemId:'i1'})} as any)
    res = await POST(req as any)
    expect((await res.json()).skipped).toBe('stock ok')

    // 6. already alerted
    vi.mocked(supa.createClient).mockReturnValue(mockService(item, {id:'b1'}, {id:'log1'}, null, null) as any)
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({itemId:'i1'})} as any)
    res = await POST(req as any)
    expect((await res.json()).skipped).toMatch(/already alerted/)

    // 7. telegram/viber/whatsapp branches + email fallback via owner_id
    const bizFull = { owner_id:'o1', name:'Biz', email:null, telegram_bot_token:'tok', telegram_chat_id:'tc', viber_bot_token:'vbt', viber_chat_id:'vc', owner_whatsapp:'+123' }
    vi.mocked(supa.createClient).mockReturnValue(mockService(item, {id:'b1'}, null, bizFull, {user:{email:'owner@test.com'}}) as any)
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({itemId:'i1'})} as any)
    res = await POST(req as any)
    ...
    const bizNoEmail = { owner_id:'o1', name:'Biz', email:null, telegram_bot_token:null, viber_bot_token:null, owner_whatsapp:null }
    vi.mocked(supa.createClient).mockReturnValue(mockService(item, {id:'b1'}, null, bizNoEmail, {user:null}) as any)
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({itemId:'i1'})} as any)
    res = await POST(req as any)
    expect((await res.json()).email).toMatch(/skipped: no email/)

    // 9. success with direct email (no fallback)
    const bizEmail = { owner_id:'o1', name:'Biz', email:'biz@test.com', telegram_bot_token:null, viber_bot_token:null, owner_whatsapp:null }
    vi.mocked(supa.createClient).mockReturnValue(mockService(item, {id:'b1'}, null, bizEmail, null) as any)
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({itemId:'i1'})} as any)
    res = await POST(req as any)
    expect((await res.json()).sent).toBe(true)
    ...
    vi.mocked(supa.createClient).mockReturnValue({
      from: vi.fn(()=>{ throw new Error('db boom') }),
      auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1'}}, error:null})), admin:{ getUserById: vi.fn(async()=>({data:null, error:null})) } },
    } as any)
    req = new NextRequest('http://localhost/api/email/low-stock', { method:'POST', body: JSON.stringify({itemId:'i1'})} as any)
    res = await POST(req as any)
  ...
  it('book covers outside_hours/break/closed and creation paths', async()=>{
    ...
    const nextMonday = new Date(); nextMonday.setDate(nextMonday.getDate()+ (8 - nextMonday.getDay())%7)
    const dateStr = nextMonday.toISOString().slice(0,10)
    const clientsChain = { select: vi.fn(()=>({ eq: vi.fn(()=>({ or: vi.fn(()=>({ limit: vi.fn(async()=>({data:[], error:null})) })) })) })) , insert: vi.fn(()=>({ select: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'c1'}, error:null})) })) })) } as any
    const apptChain = { insert: vi.fn(()=>({ select: vi.fn(()=>({ single: vi.fn(async()=>({data:{id:'a1'}, error:null})) })) })) } as any
    vi.mocked(createServiceClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        ...
        if(t==='business_hours') return { select: vi.fn(()=>({ eq: vi.fn(async()=>({data:[{day_of_week: nextMonday.getDay(), is_open:false, open_time:'09:00', close_time:'18:00', break_start:null, break_end:null}], error:null})) })) } as any
    ...
    const allDays = Array.from({length:7}, (_,i)=> ({day_of_week:i, is_open:true, open_time:'09:00', close_time:'12:00', break_start:null, break_end:null}))
    vi.mocked(createServiceClient).mockReturnValue({
      from: vi.fn((t:string)=>{
        if(t==='services') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:service, error:null})) })) })) })) })) } as any
        if(t==='businesses') return { select: vi.fn(()=>({ eq: vi.fn(()=>({ maybeSingle: vi.fn(async()=>({data:biz, error:null})) })) })) } as any
    ...
    req = new NextRequest('http://localhost/api/book', { method:'POST', body: JSON.stringify({ businessId:'11111111-1111-1111-1111-111111111111', serviceId:'22222222-2222-2222-2222-222222222222', date: dateStr, time:'13:00', name:'John', phone:'+123'})} as any)
    res = await POST(req as any)
    expect(res.status).toBe(400)
  })
  ...
  it('covers register slug collision and onboarding business not found', async()=>{
    ...
    vi.mocked(createClient).mockResolvedValue({ auth:{ signUp: vi.fn(async()=>({data:{user:{id:'u1', email:'a@b.com', user_metadata:{full_name:'Test'}}, session:{access_token:'tok'}}, error:null})) } } as any)
    let attempt=0
    ...
    const fd = new FormData(); fd.set('email','a@b.com'); fd.set('password','12345678'); fd.set('business_name','Test')
  ...
  it('covers proxy protected and locale and auth pages', async()=>{
    ...
    vi.mocked(createServerClient).mockReturnValue({ auth:{ getUser: vi.fn(async()=>({data:{user:null}, error:null})) } } as any)
    ...
    ;(url2 as any).clone = ()=>{ const c=new URL(url2.toString()); (c as any).clone=(url2 as any).clone; return c }
    req = {
      nextUrl: url2,
      cookies: { get: vi.fn(()=>null), getAll: vi.fn(()=>[]), set: vi.fn() },
      headers: new Headers(),
    ...
    vi.mocked(createServerClient).mockReturnValue({ auth:{ getUser: vi.fn(async()=>({data:{user:{id:'u1', email:'a@b.com'}}, error:null})) } } as any)
    const url3 = new URL('http://localhost/login')
    ;(url3 as any).clone = ()=>{ const c=new URL(url3.toString()); (c as any).clone=(url3 as any).clone; return c }
    req = {
      nextUrl: url3,
      cookies: { get: vi.fn(()=>null), getAll: vi.fn(()=>[]), set: vi.fn() },
      headers: new Headers(),
    ...
    ;(url4 as any).clone = ()=>{ const c=new URL(url4.toString()); (c as any).clone=(url4 as any).clone; return c }
    req = {
      nextUrl: url4,
      cookies: { get: vi.fn(()=>null), getAll: vi.fn(()=>[]), set: vi.fn() },
    ...
    ;(url5 as any).clone = ()=>{ const c=new URL(url5.toString()); (c as any).clone=(url5 as any).clone; return c }
    req = {
      nextUrl: url5,
      cookies: { get: vi.fn(()=>null), getAll: vi.fn(()=>[]), set: vi.fn() },
      headers: new Headers(),
