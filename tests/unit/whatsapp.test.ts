...
describe('whatsapp exhaustive', () => {
  ...
  it('send', async () => {
    process.env.META_WHATSAPP_PHONE_NUMBER_ID = 'id'
    process.env.META_WHATSAPP_ACCESS_TOKEN = 'tok'
    expect(
      await sendWhatsAppMessage('123', 'hi', { phoneNumberId: 'id', accessToken: 'tok' }),
    ).toBe(true)
    expect(await sendWhatsAppMessage('123', 'hi')).toBe(true)
  })
  it('tpl', () => {
    expect(
      tplBookingConfirmation({
