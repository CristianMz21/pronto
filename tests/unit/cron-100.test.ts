...
describe('cron 100', () => {
  ...
  it('covers all 5 windows', async () => {
    ...
    const biz = { name:'Biz', address:'Addr', slug:'biz', timezone:'UTC', telegram_bot_token:'tg', telegram_chat_id:'tc', viber_bot_token:'vb', viber_chat_id:'vc', meta_whatsapp_phone_number_id:'pid', meta_whatsapp_access_token:'tok' }
    const appt = { id:'a1', starts_at: new Date(now.getTime()+24*3600*1000).toISOString(), business_id:'b1', services:{name:'Cut'}, employees:{name:'John'}, clients:{name:'Client', email:'c@test.com', whatsapp_number:'123', viber_user_id:'v1', telegram_id:'t1'} }
    const appt1h = { ...appt, id:'a2', starts_at: new Date(now.getTime()+60*60*1000).toISOString() }
    const completed = { id:'a3', business_id:'b1', services:{name:'Cut'}, clients:{name:'Client', email:'c@test.com', whatsapp_number:'123', viber_user_id:'v1', telegram_id:'t1'} }
    const dormant = { id:'c1', name:'Dormant', email:'d@test.com', whatsapp_number:'123', viber_user_id:'v1', telegram_id:'t1', business_id:'b1', last_visit_at: new Date(now.getTime()-30*24*3600*1000).toISOString() }
    const bdayClient = { id:'c2', name:'Bday', email:'b@test.com', whatsapp_number:'123', viber_user_id:'v1', telegram_id:'t1', business_id:'b1', birthday:`2000-${todayMD}` }
    ...
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((table:string)=>{
        ...
        const c:any={}
        ;['select','gte','lte','eq','not','is','single','insert'].forEach(m=> c[m]=vi.fn(()=>c))
      ...
      return {
        from: vi.fn((table:string)=>{
          if(table==='appointments'){
            apptCall++
            if(apptCall===1){
              const c:any={}; ['select','gte','lte','eq'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[appt], error:null}).then(r); return c
            }
            if(apptCall===2){
              const c:any={}; ['select','gte','lte','eq'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[appt1h], error:null}).then(r); return c
            }
            if(apptCall===3){
              const c:any={}; ['select','eq','gte','lte'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[completed], error:null}).then(r); return c
            }
          }
          if(table==='clients'){
            clientQueryCount++
            if(clientQueryCount===1){
              // reactivation
              const c:any={}; ['select','gte','lte'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[dormant], error:null}).then(r); return c
            } else {
              // birthday
              const c:any={}; ['select','not','is'].forEach(m=> c[m]=vi.fn(()=>c)); c.not=vi.fn(()=>({ is: vi.fn(async()=>({data:[bdayClient], error:null})) })); return c
            }
          }
          if(table==='businesses'){
            return { select: vi.fn(()=>({ eq: vi.fn(()=>({ single: vi.fn(async()=>({data: biz, error:null})) })) })) } as any
  ...
  it('cron handles no contact skip and already logged', async () => {
    process.env.CRON_SECRET='secret'
    const { createClient } = await import('@supabase/supabase-js')
    const apptNoContact = { id:'a1', starts_at: new Date(Date.now()+24*3600*1000).toISOString(), business_id:'b1', services:{name:'Cut'}, employees:{name:'John'}, clients:{name:'NoContact', email:null, whatsapp_number:null, viber_user_id:null, telegram_id:null} }
    vi.mocked(createClient).mockReturnValue({
      from: vi.fn((table:string)=>{
        if(table==='appointments'){
          const c:any={}; ['select','gte','lte','eq'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[apptNoContact], error:null}).then(r); return c
        }
        if(table==='notification_log'){
          return { insert: vi.fn(async()=>({error:{code:'23505', message:'dup'}})) } as any
        }
        const c:any={}; ['select','gte','lte','eq','not','is'].forEach(m=> c[m]=vi.fn(()=>c)); c.then=(r:any)=> Promise.resolve({data:[], error:null}).then(r); return c
      }),
    } as any)
    const { GET } = await import('@/app/api/cron/notify/route')
    const req = new NextRequest('http://localhost/api/cron/notify', { headers:{ authorization:'Bearer secret'} } as any)
