...
export function isEligible(
  cm: Pick<ClientMembership, 'remaining' | 'expires_at' | 'status'>,
  now: Date = new Date(),
): boolean {
...
export async function purchaseMembership(
  supabase: SupabaseLike & {
    from: (t: string) => {
      ...
      insert: (data: unknown) => {
        select: (...args: unknown[]) => { single: () => Promise<{ data: unknown; error: unknown }> }
      }
    }
  },
  ...
  const { data: membership, error: mErr } = await (
      ...
      select: (...args: unknown[]) => {
        eq: (
          ...
          eq: (
            c2: string,
            v2: unknown,
          ) => { maybeSingle: () => Promise<{ data: Membership | null; error: unknown }> }
        }
      }
    }
  )
