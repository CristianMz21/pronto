        ...
        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
          <tr>
            <td style="background:#2563eb;padding:20px 32px;">
              <span style="color:#ffffff;font-size:20px;font-weight:700;">${safeName}</span>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="background:#f9fafb;padding:16px 32px;border-top:1px solid #e5e7eb;">
              <p style="margin:0;font-size:12px;color:#9ca3af;">
                Powered by <a href="${escapeAttr(APP_URL)}" style="color:#2563eb;text-decoration:none;">Pronto</a>
              </p>
            </td>
          </tr>
        </table>
...
function btn(text: string, href: string) {
  return `<a href="${escapeAttr(href)}" style="display:inline-block;margin-top:20px;padding:12px 24px;background:#2563eb;color:#fff;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none;">${escapeHtml(text)}</a>`
}

function h1(text: string) {
  return `<h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#111827;">${escapeHtml(text)}</h1>`
}
...
export async function sendBookingConfirmation(opts: {
  ...
  const body = `
    ...
    ${p(`Hi ${escapeHtml(firstName(opts.clientName))}, your appointment is confirmed.`)}
    ${info([
    ...
    ${p('See you soon!')}
    ${opts.calendarUrl ? p(`<a href="${safeCalendarUrl}" style="color:#2563eb;">Add to Google Calendar</a>`) : ''}
  `
  return sendMail({
...
export async function sendReminder(opts: {
  ...
  const body = `
    ...
    ${p(`Hi ${escapeHtml(firstName(opts.clientName))}, just a friendly reminder about your upcoming appointment.`)}
    ${info([
      ['Service', opts.serviceName],
      ['Date', opts.date],
      ['Time', opts.time],
...
export async function sendThankYou(opts: {
  ...
  const body = `
    ...
    ${p(`Hi ${escapeHtml(firstName(opts.clientName))}, thank you for choosing ${escapeHtml(opts.businessName)}. We hope to see you again!`)}
    ${p(`You were in for: <strong>${escapeHtml(opts.serviceName)}</strong>`)}
...
export async function sendReactivation(opts: {
  ...
  const body = `
    ${h1('We miss you!')}
    ${p(`Hi ${escapeHtml(firstName(opts.clientName))}, it's been a while since your last visit to ${escapeHtml(opts.businessName)}.`)}
...
export async function sendBirthday(opts: {
  ...
  const body = `
    ${h1('🎂 Happy Birthday!')}
    ${p(`Hi ${escapeHtml(firstName(opts.clientName))}, wishing you a wonderful birthday from the whole team at ${escapeHtml(opts.businessName)}!`)}
...
export async function sendLowStockAlert(opts: {
  ...
  const body = `
    ${h1('Low-stock alert')}
    ${p(`The following items in ${escapeHtml(opts.businessName)} are running low:`)}
    ${info(rows)}
