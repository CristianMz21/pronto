...
export const EnqueueSchema = z.object({
  ...
  status: z.enum(['waiting', 'notified', 'converted', 'expired', 'cancelled']).optional().default('waiting'),
})
...
export function isExpired(entry: Pick<WaitlistEntry, 'status' | 'notified_at'>, now: Date = new Date()): boolean {
...
export function canEnqueue(desiredAt: string, now: Date = new Date(), minAdvanceMinutes = 30, enabled = true): { ok: true } | { ok: false; reason: string } {
...
export async function enqueue(
  ...
  const { data, error } = await (supabase.from('waitlist') as unknown as {
    insert: (d: unknown) => { select: (c: string) => { single: () => Promise<{ data: WaitlistEntry | null; error: unknown }> } }
...
export async function listWaiting(
  ...
  let q = (supabase.from('waitlist') as unknown as {
    select: (c: string) => { eq: (col: string, v: unknown) => { eq: (col: string, v: unknown) => { order: (col: string, o: unknown) => { limit: (n: number) => Promise<{ data: WaitlistEntry[] | null; error: unknown }> } } } }
  }).select('*, clients(id, name, phone, email), services(id, name), employees(id, name)').eq('business_id', businessId).eq('status', 'waiting').order('created_at', { ascending: true }).limit(limit)

  // Location filter if provided
  if (opts?.location_id) {
    // Supabase chaining: need to handle dynamic — fetch all then filter for simplicity in V1, or add eq if available
    const { data } = await q
    const filtered = (data ?? []).filter((w) => !w.location_id || w.location_id === opts.location_id)
    return filtered
  }
...
export async function notifyNext(
  supabase: SupabaseLike,
  params: { business_id: string; desired_at?: string; location_id?: string | null; service_id?: string | null; employee_id?: string | null }
  ...
  const supa = supabase as unknown as {
    from: (t: string) => {
      ...
      update: (d: unknown) => {
        eq: (a: string, b: unknown) => {
          select: (c: string) => { single: () => Promise<{ data: WaitlistEntry | null; error: unknown }> }
        }
      }
      rpc?: unknown
    }
  ...
  const base = supa.from('waitlist').select('*').eq('business_id', business_id).eq('status', 'waiting').order('created_at', { ascending: true }).limit(50)
  ...
  if (desired_at) {
    ...
    if (filtered.length === 0) {
      ...
      filtered = (candidates as WaitlistEntry[]).filter((w) => w.desired_at.slice(0, 10) === targetDay)
    }
  }
  ...
  if (employee_id !== undefined) {
    if (employee_id) filtered = filtered.filter((w) => !w.employee_id || w.employee_id === employee_id)
  }
...
export async function convert(
  ...
  if (!parsed.success) throw Object.assign(new Error('validation_failed'), { code: 'validation_failed' })

  const supa = supabase as unknown as {
    from: (t: string) => {
      update: (d: unknown) => { eq: (a: string, b: unknown) => { select: (c: string) => { single: () => Promise<{ data: WaitlistEntry | null; error: unknown }> } } }
      select: (c: string) => { eq: (a: string, b: unknown) => { maybeSingle: () => Promise<{ data: WaitlistEntry | null; error: unknown }> } }
    }
  }
  // Verify current status allows conversion (waiting or notified)
  const { data: current } = await (supa.from('waitlist') as unknown as {
    select: (c: string) => { eq: (a: string, b: unknown) => { maybeSingle: () => Promise<{ data: WaitlistEntry | null; error: unknown }> } }
  }).select('status').eq('id', waitlistId).maybeSingle()
  if (!current) throw Object.assign(new Error('waitlist_not_found'), { code: 'waitlist_not_found' })
  if ((current as WaitlistEntry).status === 'converted') throw Object.assign(new Error('already_converted'), { code: 'already_converted' })
  if ((current as WaitlistEntry).status === 'expired' || (current as WaitlistEntry).status === 'cancelled') {
    throw Object.assign(new Error('waitlist_not_convertible'), { code: 'waitlist_not_convertible' })
  }

  const { data, error } = await supa.from('waitlist').update({ status: 'converted' }).eq('id', waitlistId).select('*').single()
  if (error) throw error
  return data as WaitlistEntry
}
...
export async function expireStale(
  ...
  const supa = supabase as unknown as {
    from: (t: string) => {
      select: (c: string) => { eq: (col: string, v: unknown) => { lt: (col: string, v: unknown) => Promise<{ data: WaitlistEntry[] | null; error: unknown }> } & { in: (col: string, v: unknown[]) => Promise<{ data: WaitlistEntry[] | null; error: unknown }> } }
      update: (d: unknown) => { in: (col: string, v: unknown[]) => Promise<{ data: unknown; error: unknown }> }
    }
  }
  ...
  const { data: notifiedCandidates } = await (supabase.from('waitlist') as unknown as {
    select: (c: string) => { eq: (a: string, b: unknown) => { lt: (col: string, v: unknown) => Promise<{ data: WaitlistEntry[] | null; error: unknown }> } }
  }).select('id').eq('status', 'notified').lt('notified_at', cutoff) as unknown as Promise<{ data: WaitlistEntry[] | null; error: unknown }>

  let expiredNotified = 0
  if (notifiedCandidates && notifiedCandidates.length > 0) {
    const ids = notifiedCandidates.map((r) => r.id)
    await (supa.from('waitlist').update({ status: 'expired' } as unknown as never) as unknown as { in: (c: string, v: unknown[]) => Promise<unknown> }).in('id', ids)
    expiredNotified = ids.length
  }

  // Waiting past desired_at (desired_at < now)
  const { data: waitingPast } = await (supabase.from('waitlist') as unknown as {
    select: (c: string) => { eq: (a: string, b: unknown) => { lt: (col: string, v: unknown) => Promise<{ data: WaitlistEntry[] | null; error: unknown }> } }
  }).select('id').eq('status', 'waiting').lt('desired_at', nowIso) as unknown as Promise<{ data: WaitlistEntry[] | null; error: unknown }>
